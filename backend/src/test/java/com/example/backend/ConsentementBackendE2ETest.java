...
class ConsentementBackendE2ETest extends BaseBackendE2ETest {
    ...
    void postConsentement_withSmsChannel_createsConsentWithInitialStatus() throws Exception {
        ...
        mockMvc.perform(
                        withTenantHeaders(post("/api/v1/consentements"), ORG_ID, CORRELATION_ID)
    ...
    void putConsentement_updatesStatusAndStoresPreviousStatusInMeta() throws Exception {
        ...
        ConsentementEntity consent =
                createConsent(dossier, ConsentementChannel.EMAIL, ConsentementStatus.PENDING);
        ...
        mockMvc.perform(
                        withTenantHeaders(
                                        put("/api/v1/consentements/" + consent.getId()),
                                        ORG_ID,
                                        CORRELATION_ID)
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void getConsentements_withDossierId_returnsAllConsentsForDossier() throws Exception {
        ...
        mockMvc.perform(
                        withTenantHeaders(get("/api/v1/consentements"), ORG_ID, CORRELATION_ID)
    ...
    void getConsentements_sortedByUpdatedAtDesc_returnsMostRecentFirst() throws Exception {
        ...
        ConsentementEntity oldest =
                createConsent(dossier, ConsentementChannel.EMAIL, ConsentementStatus.PENDING);
        Thread.sleep(10);
        ConsentementEntity middle =
                createConsent(dossier, ConsentementChannel.SMS, ConsentementStatus.GRANTED);
        Thread.sleep(10);
        ConsentementEntity newest =
                createConsent(dossier, ConsentementChannel.WHATSAPP, ConsentementStatus.DENIED);
    ...
    void getConsentements_afterUpdate_maintainsDescendingOrder() throws Exception {
        ...
        ConsentementEntity consent1 =
                createConsent(dossier, ConsentementChannel.EMAIL, ConsentementStatus.PENDING);
        Thread.sleep(10);
        ConsentementEntity consent2 =
                createConsent(dossier, ConsentementChannel.SMS, ConsentementStatus.GRANTED);
        Thread.sleep(10);
        ...
        mockMvc.perform(
                        withTenantHeaders(
                                        put("/api/v1/consentements/" + consent1.getId()),
                                        ORG_ID,
                                        CORRELATION_ID)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(updateRequest)))
    ...
    void putConsentement_multipleStatusTransitions_createsMultipleAuditEvents() throws Exception {
        ...
        ConsentementEntity consent =
                createConsent(dossier, ConsentementChannel.WHATSAPP, ConsentementStatus.PENDING);
        ...
        Map<String, Object> firstChanges =
                (Map<String, Object>) firstAudit.getDiff().get("changes");
        @SuppressWarnings("unchecked")
        ...
        Map<String, Object> secondChanges =
                (Map<String, Object>) secondAudit.getDiff().get("changes");
        @SuppressWarnings("unchecked")
    ...
    void putConsentement_withCustomMeta_mergesWithTrackingMeta() throws Exception {
        ...
        ConsentementEntity consent =
                createConsentWithMeta(
                        dossier,
                        ConsentementChannel.EMAIL,
                        ConsentementStatus.PENDING,
                        initialMeta);
    ...
    @Test
    void getConsentements_withPaginationAndChannelFilter_returnsFilteredPagedResults()
            throws Exception {
    ...
    void getConsentements_paginatedResults_maintainDescendingOrderByUpdatedAt() throws Exception {
        ...
        List<ConsentementEntity> consents =
                consentementRepository.findByDossierIdOrderByUpdatedAtDesc(dossier.getId());
    ...
    private ConsentementEntity createConsentWithMeta(
            Dossier dossier,
            ConsentementChannel channel,
            ConsentementStatus status,
