...
public class DataSourceHealthCheck implements ApplicationListener<ApplicationStartedEvent> {
    ...
    public void onApplicationEvent(ApplicationStartedEvent event) {
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            try (ResultSet tables = metaData.getTables(null, null, "flyway_schema_history", null)) {
                if (tables.next()) {
                    ...
                    try (var stmt = connection.createStatement();
                         var rs = stmt.executeQuery("SELECT COUNT(*) as count FROM flyway_schema_history WHERE success = true")) {
                        if (rs.next()) {
                    ...
                    log.warn("⚠ Flyway schema history table not found - migrations may not have run");
                }
            }
            ...
            log.info("╔════════════════════════════════════════════════════════════════════════════╗");
            log.info("║ ✓ DataSource Health Check: PASSED                                          ║");
            log.info("╚════════════════════════════════════════════════════════════════════════════╝");
            
        } catch (Exception e) {
            log.error("╔════════════════════════════════════════════════════════════════════════════╗");
            log.error("║ ✗ DataSource Health Check: FAILED                                          ║");
            log.error("╚════════════════════════════════════════════════════════════════════════════╝");
