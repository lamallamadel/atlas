...
class MessageBackendE2ETest extends BaseBackendE2ETest {
    ...
    void createMessage_AllChannelsAndDirectionsCombinations_Returns201() throws Exception {
        ...
        MessageChannel[] channels = {
            MessageChannel.EMAIL, MessageChannel.SMS, MessageChannel.WHATSAPP, MessageChannel.PHONE
        };
    ...
    void createMessage_AuditLog_ContainsContentInDiff() throws Exception {
        ...
        List<AuditEventEntity> auditEvents =
                auditEventRepository.findAll(Sort.by(Sort.Direction.DESC, "createdAt"));
        assertThat(auditEvents).hasSizeGreaterThanOrEqualTo(1);

        AuditEventEntity messageAudit =
                auditEvents.stream()
                        .filter(
                                a ->
                                        a.getEntityType() == AuditEntityType.MESSAGE
                                                && a.getEntityId().equals(messageId))
                        .findFirst()
                        .orElseThrow();
                                    ...
                                    a -> {
                                        ...
                                        if (diff != null && diff.containsKey("after")) {
                                            Map<String, Object> after =
                                                    (Map<String, Object>) diff.get("after");
                                            return after.get("content")
                                                    .equals("Message content " + finalI);
                                        }
                                        return false;
                                    })
                            .findFirst()
    ...
    void createMessage_AuditLog_IsolatedByTenant() throws Exception {
        ...
        List<AuditEventEntity> tenant1Audits =
                allAudits.stream()
                        .filter(a -> a.getOrgId().equals(TENANT_1))
                        .filter(a -> a.getEntityType() == AuditEntityType.MESSAGE)
                        .toList();

        List<AuditEventEntity> tenant2Audits =
                allAudits.stream()
        ...
        Map<String, Object> tenant1After =
                (Map<String, Object>) tenant1Audits.get(0).getDiff().get("after");
        assertThat(tenant1After.get("content")).isEqualTo("Tenant 1 message");

        Map<String, Object> tenant2After =
                (Map<String, Object>) tenant2Audits.get(0).getDiff().get("after");
        ...
        mockMvc.perform(
                        withTenantHeaders(
                                get("/api/v1/messages")
                                        ...
                                        .param("endDate", "2024-01-15T23:59:59"),
                                TENANT_1))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(3)));
    }
        ...
        mockMvc.perform(
                        withTenantHeaders(
                                get("/api/v1/messages")
                                        .with(jwtWithRoles(TENANT_1))
                                        .param("dossierId", dossier.getId().toString())
                                        .param("startDate", "2024-01-10T00:00:00"),
                                TENANT_1))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(2)));
    }
        ...
        mockMvc.perform(
                        withTenantHeaders(
                                get("/api/v1/messages")
                                        .with(jwtWithRoles(TENANT_1))
                                        .param("dossierId", dossier.getId().toString())
                                        .param("endDate", "2024-01-10T10:00:00"),
                                TENANT_1))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content", hasSize(3)));
    }
    ...
    @Test
    void listMessages_FilterByDateRangeChannelAndDirection_ReturnsCombinedFilter()
            throws Exception {
        Dossier dossier = createDossier(TENANT_1);
        ...
        createMessage(
                ...
                MessageChannel.EMAIL,
                MessageDirection.OUTBOUND,
                LocalDateTime.of(2024, 1, 10, 11, 0));
        createMessage(
                dossier,
                MessageChannel.SMS,
                MessageDirection.INBOUND,
        ...
        try {
            MvcResult result =
                    mockMvc.perform(
                                    withTenantHeaders(
                                            post("/api/v1/messages")
                                                    .with(jwtWithRoles(dossier.getOrgId()))
                                                    .contentType(MediaType.APPLICATION_JSON)
                                                    .content(
