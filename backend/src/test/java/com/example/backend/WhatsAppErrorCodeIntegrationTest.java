...
class WhatsAppErrorCodeIntegrationTest extends BaseBackendE2ETest {
    ...
    void testRetryableErrorCodes(String errorCode, String errorMessage, boolean expectedRetryable) {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(ProviderSendResult.failure(errorCode, errorMessage, true, null));
        ...
        assertThat(updated.getStatus())
                .withFailMessage(
                        "Error code %s should result in QUEUED status for retry", errorCode)
    ...
    void testNonRetryableErrorCodes(
            String errorCode, String errorMessage, boolean expectedRetryable) {
        TenantContext.setOrgId(TENANT_1);

        OutboundMessageEntity message = createQueuedMessage();

        when(mockWhatsAppProvider.send(any()))
                .thenReturn(ProviderSendResult.failure(errorCode, errorMessage, false, null));
        ...
        assertThat(updated.getStatus())
                .withFailMessage(
                        "Error code %s should result in FAILED status immediately", errorCode)
    ...
    void testRateLimitErrorCodes(String errorCode, String errorMessage, boolean expectedRateLimit) {
        ...
        assertThat(isRateLimitError)
                .withFailMessage(
                        "Error code %s should be identified as rate limit error", errorCode)
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(
                        ProviderSendResult.failure(
                                "133008", "Template parameter count mismatch", false, null));
    ...
    void testSessionWindowError_Outside24Hours() {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(
                        ProviderSendResult.failure(
                                "132015", "Cannot send message after 24 hour window", false, null));
    ...
    void testAccountError_Blocked() {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(
                        ProviderSendResult.failure(
                                "132068",
                                "Business account blocked from sending messages",
                                false,
                                null));
    ...
    void testRecipientError_NotOnWhatsApp() {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(
                        ProviderSendResult.failure(
                                "131021", "Recipient not on WhatsApp", false, null));
    ...
    void testRecipientError_BlockedSender() {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(ProviderSendResult.failure("131031", "Recipient blocked", false, null));
    ...
    void testContentError_MessageTooLong() {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(ProviderSendResult.failure("131045", "Message too long", false, null));
    ...
    void testContentError_SpamFilter() {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(
                        ProviderSendResult.failure(
                                "132007", "Message blocked by spam filter", false, null));
    ...
    void testErrorProgression_RetryableEventuallyFails() {
        ...
        when(mockWhatsAppProvider.send(any()))
                .thenReturn(
                        ProviderSendResult.failure(
                                "131016", "Service temporarily unavailable", true, null));
        ...
        attempts.forEach(
                attempt -> assertThat(attempt.getStatus()).isEqualTo(OutboundAttemptStatus.FAILED));
    }
