 ...
 * 
 * This test is designed to isolate the repository layer and diagnose issues with
 * findByDossierIdWithFilters() method by:
 * 1. Using @DataJpaTest for repository-only testing
 * 2. Using @AutoConfigureTestDatabase(replace = NONE) to use real PostgreSQL
 * 3. Executing queries directly with known test data
 * 4. Capturing exact SQLException or ConstraintViolationException details
 */
@DataJpaTest
...
class MessageRepositoryPostgresIT {
    ...
    void setUp() {
        ...
        log.info("Created test dossier with ID: {}, orgId: {}", testDossier.getId(), testDossier.getOrgId());

        // Create test messages with different attributes
        testMessage1 = createMessage(testDossier, MessageChannel.EMAIL, MessageDirection.INBOUND, 
                "Test email message", LocalDateTime.now().minusDays(2));
        testMessage1 = messageRepository.saveAndFlush(testMessage1);
        log.info("Created test message 1 - ID: {}, Channel: {}, Direction: {}, Timestamp: {}", 
                testMessage1.getId(), testMessage1.getChannel(), testMessage1.getDirection(), testMessage1.getTimestamp());

        testMessage2 = createMessage(testDossier, MessageChannel.SMS, MessageDirection.OUTBOUND, 
                "Test SMS message", LocalDateTime.now().minusDays(1));
        testMessage2 = messageRepository.saveAndFlush(testMessage2);
        log.info("Created test message 2 - ID: {}, Channel: {}, Direction: {}, Timestamp: {}", 
                testMessage2.getId(), testMessage2.getChannel(), testMessage2.getDirection(), testMessage2.getTimestamp());

        testMessage3 = createMessage(testDossier, MessageChannel.WHATSAPP, MessageDirection.INBOUND, 
                "Test WhatsApp message", LocalDateTime.now());
        testMessage3 = messageRepository.saveAndFlush(testMessage3);
        log.info("Created test message 3 - ID: {}, Channel: {}, Direction: {}, Timestamp: {}", 
                testMessage3.getId(), testMessage3.getChannel(), testMessage3.getDirection(), testMessage3.getTimestamp());
    ...
    void testFindByDossierIdWithFilters_NoFilters() {
        ...
        try {
            log.info("Executing query: dossierId={}, channel=null, direction=null, startDate=null, endDate=null", 
                    testDossier.getId());
            
            Page<MessageEntity> result = messageRepository.findByDossierIdWithFilters(
                    testDossier.getId(), null, null, null, null, pageable);
            
            log.info("Query executed successfully");
            log.info("Total elements: {}, Total pages: {}, Content size: {}", 
            ...
            result.getContent().forEach(msg -> 
                log.info("Found message - ID: {}, Channel: {}, Direction: {}, Timestamp: {}", 
                        msg.getId(), msg.getChannel(), msg.getDirection(), msg.getTimestamp()));
            
            log.info("✓ Test passed: All 3 messages retrieved successfully");
            
        } catch (DataIntegrityViolationException e) {
            log.error("❌ DataIntegrityViolationException caught", e);
            log.error("Root cause: {}", e.getRootCause() != null ? e.getRootCause().getMessage() : "null");
            ...
            e.getConstraintViolations().forEach(violation -> 
                log.error("Constraint violation - Property: {}, Value: {}, Message: {}", 
    ...
    void testFindByDossierIdWithFilters_FilterByChannel() {
        ...
        try {
            log.info("Executing query: dossierId={}, channel=EMAIL, direction=null, startDate=null, endDate=null", 
                    testDossier.getId());
            ...
            log.info("Query executed successfully");
            log.info("Total elements: {}, Content size: {}", 
            ...
            log.info("Found message - ID: {}, Channel: {}, Direction: {}", 
    ...
    void testFindByDossierIdWithFilters_FilterByDirection() {
        ...
        try {
            log.info("Executing query: dossierId={}, channel=null, direction=INBOUND, startDate=null, endDate=null", 
                    testDossier.getId());
            
            Page<MessageEntity> result = messageRepository.findByDossierIdWithFilters(
                    testDossier.getId(), null, MessageDirection.INBOUND, null, null, pageable);
            ...
            assertThat(result.getContent()).allMatch(msg -> msg.getDirection() == MessageDirection.INBOUND);
            
            result.getContent().forEach(msg -> 
                log.info("Found message - ID: {}, Channel: {}, Direction: {}", 
                        msg.getId(), msg.getChannel(), msg.getDirection()));
    ...
    void testFindByDossierIdWithFilters_FilterByDateRange() {
        ...
        try {
            log.info("Executing query: dossierId={}, channel=null, direction=null, startDate={}, endDate={}", 
                    testDossier.getId(), startDate, endDate);
            ...
            assertThat(result.getContent()).allMatch(msg -> 
                !msg.getTimestamp().isBefore(startDate) && !msg.getTimestamp().isAfter(endDate));
            
            result.getContent().forEach(msg -> 
                log.info("Found message - ID: {}, Timestamp: {}", msg.getId(), msg.getTimestamp()));
    ...
    void testFindByDossierIdWithFilters_MultipleFilters() {
        ...
        try {
            log.info("Executing query: dossierId={}, channel=WHATSAPP, direction=INBOUND, startDate={}, endDate={}", 
                    testDossier.getId(), startDate, endDate);
            
            Page<MessageEntity> result = messageRepository.findByDossierIdWithFilters(
                    testDossier.getId(), MessageChannel.WHATSAPP, MessageDirection.INBOUND, 
            ...
            log.info("Found message - ID: {}, Channel: {}, Direction: {}, Timestamp: {}", 
                    foundMessage.getId(), foundMessage.getChannel(), 
                    foundMessage.getDirection(), foundMessage.getTimestamp());
    ...
    void testFindByDossierIdWithFilters_EmptyResult() {
        ...
        try {
            log.info("Executing query: dossierId={}, channel=PHONE, direction=null, startDate=null, endDate=null", 
                    testDossier.getId());
    ...
    void testFindByDossierIdWithFilters_Pagination() {
        ...
        for (int i = 0; i < 7; i++) {
            MessageEntity msg = createMessage(testDossier, MessageChannel.EMAIL, MessageDirection.INBOUND,
                    "Additional message " + i, LocalDateTime.now().plusMinutes(i));
        ...
        try {
            ...
            log.info("Query executed successfully");
            log.info("Total elements: {}, Total pages: {}, Content size: {}, Current page: {}", 
                    result.getTotalElements(), result.getTotalPages(), result.getContent().size(), result.getNumber());
    ...
    void testFindByDossierIdWithFilters_VerifyJoinFetch() {
        ...
        try {
            ...
            log.info("Dossier loaded eagerly - ID: {}, OrgId: {}", dossier.getId(), dossier.getOrgId());
    ...
    private MessageEntity createMessage(Dossier dossier, MessageChannel channel, 
                                       MessageDirection direction, String content, LocalDateTime timestamp) {
    ...
    private void logExceptionDetails(Exception e) {
        ...
        if (e instanceof DataIntegrityViolationException) {
            ...
            log.error("Root cause: {}", dive.getRootCause() != null ? dive.getRootCause().getMessage() : "null");
        ...
        if (e instanceof org.hibernate.exception.ConstraintViolationException) {
            org.hibernate.exception.ConstraintViolationException hcve = (org.hibernate.exception.ConstraintViolationException) e;
        ...
        if (e.getCause() != null) {
            ...
            if (e.getCause() instanceof SQLException) {
                SQLException sqlEx = (SQLException) e.getCause();
                log.error("SQL error details - SQLState: {}, ErrorCode: {}", 
