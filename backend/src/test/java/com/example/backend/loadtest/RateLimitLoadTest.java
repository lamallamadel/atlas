 ...
 * 
 * This test simulates multiple organizations making concurrent requests
 * to validate that rate limits are properly enforced across instances.
 * 
 * Run with: mvn gatling:test -Dgatling.simulationClass=com.example.backend.loadtest.RateLimitLoadTest
 */
public class RateLimitLoadTest extends Simulation {
    ...
    ScenarioBuilder standardTierScenario = scenario("Standard Tier Organization")
            .exec(
    ...
    ScenarioBuilder premiumTierScenario = scenario("Premium Tier Organization")
            .exec(
    ...
    ScenarioBuilder ipBasedScenario = scenario("IP-Based Rate Limiting")
            .exec(
                    http("Webhook Request")
                            ...
                            .asJson()
                            .check(
                                    status().in(200, 404, 429)
                            )
            );

    // Scenario 4: Rate limit recovery
    ScenarioBuilder recoveryScenario = scenario("Rate Limit Recovery")
            .exec(
...

    {
        setUp(
         ...
         .assertions(
                 ...
                 details("Standard Tier Organization").successfulRequests().percent().gte(80.0),

                 // Premium tier assertions
                 details("Premium Tier Organization").requestsPerSec().lte(20.0),
                 details("Premium Tier Organization").successfulRequests().percent().gte(80.0),
