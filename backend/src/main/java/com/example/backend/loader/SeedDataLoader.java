...
@Profile("dev")
@ConditionalOnProperty(name = "immo.mock.enabled", havingValue = "true", matchIfMissing = false)
@Component
public class SeedDataLoader implements CommandLineRunner {
    ...
    private List<ReferentialEntity> createReferentialsForOrg(String orgId) {
        ...
        referentials.add(
                createReferential(
                        orgId, "CASE_STATUS", "CRM_NEW", "Nouveau", "New case", order++, true));
        referentials.add(
                createReferential(
                        orgId, "CASE_STATUS", "CRM_TRIAGED", "Trié", "Triaged", order++, true));
        referentials.add(
                createReferential(
                        orgId,
                        "CASE_STATUS",
        ...
        referentials.add(
                createReferential(
                        orgId, "CASE_STATUS", "CRM_NO_SHOW", "Absent", "No show", order++, true));
        referentials.add(
                createReferential(
                        orgId,
                        "CASE_STATUS",
        ...
        referentials.add(
                createReferential(
                        ...
                        "PHONE_CALL",
                        "Appel téléphonique",
                        "Lead from phone call",
                        order++,
                        true));
        referentials.add(
                createReferential(
                        orgId, "LEAD_SOURCE", "SMS", "SMS", "Lead from SMS", order++, true));
        referentials.add(
                createReferential(
                        orgId, "LEAD_SOURCE", "EMAIL", "Email", "Lead from email", order++, true));
        referentials.add(
                createReferential(
                        orgId,
                        "LEAD_SOURCE",
        ...
        referentials.add(
                createReferential(
                        orgId, "LEAD_SOURCE", "AVITO", "Avito", "Lead from Avito", order++, true));
        referentials.add(
                createReferential(
                        orgId,
                        "LEAD_SOURCE",
        ...
        referentials.add(
                createReferential(
                        orgId, "LOSS_REASON", "OTHER", "Autre", "Other reason", order++, true));

        order = 1;
        // WON_REASON
        referentials.add(
                createReferential(orgId, "WON_REASON", "SIGNED", "Signé", "Signed", order++, true));
        referentials.add(
                createReferential(
                        orgId, "WON_REASON", "RESERVED", "Réservé", "Reserved", order++, true));
        referentials.add(
                createReferential(
                        orgId,
                        "WON_REASON",
                        ...
                        true));
        referentials.add(
                createReferential(orgId, "WON_REASON", "SOLD", "Vendu", "Sold", order++, true));
        referentials.add(
                createReferential(orgId, "WON_REASON", "RENTED", "Loué", "Rented", order++, true));
        referentials.add(
                createReferential(
                        orgId,
                        "WON_REASON",
        ...
        referentials.add(
                createReferential(
                        orgId, "WON_REASON", "OTHER", "Autre", "Other reason", order++, true));
    ...
    private void loadAnnonces() {
        ...
        annonce1.setDescription(
                "Spacious apartment with modern amenities, great views, and convenient location.");
        ...
        annonce3.setDescription(
                "Bright and spacious condo with a large terrace overlooking the city.");
        ...
        logger.info(
                "Loaded {} annonces ({} for ORG-001, {} for ORG-002)", savedAnnonces.size(), 2, 2);
    }
    ...
    private void loadDossiers() {
        ...
        List<Annonce> org001Annonces =
                allAnnonces.stream().filter(a -> ORG_001.equals(a.getOrgId())).toList();

        List<Annonce> org002Annonces =
                allAnnonces.stream().filter(a -> ORG_002.equals(a.getOrgId())).toList();
        ...
        logger.info(
                "Loaded {} dossiers ({} for ORG-001, {} for ORG-002)", savedDossiers.size(), 10, 8);
    ...
    private void loadPartiesPrenantes(List<Dossier> dossiers) {
        ...
        // Get dossiers by org
        List<Dossier> org001Dossiers =
                dossiers.stream().filter(d -> ORG_001.equals(d.getOrgId())).toList();

        List<Dossier> org002Dossiers =
                dossiers.stream().filter(d -> ORG_002.equals(d.getOrgId())).toList();

        // ORG-001 Parties prenantes
        if (org001Dossiers.size() >= 3) {
            parties.add(
                    createPartie(
                            org001Dossiers.get(0),
                            PartiePrenanteRole.BUYER,
                            "Jean",
                            "Dupont",
            ...
            parties.add(
                    createPartie(
                            org001Dossiers.get(0),
                            PartiePrenanteRole.AGENT,
                            "Claude",
                            "Leroy",
                            "claude.leroy@agency.com",
                            "+33601020304",
                            "Agency Address, Paris"));

            parties.add(
                    createPartie(
                            org001Dossiers.get(1),
                            PartiePrenanteRole.TENANT,
                            "Marie",
                            "Martin",
            ...
            parties.add(
                    createPartie(
                            org001Dossiers.get(1),
                            PartiePrenanteRole.LANDLORD,
                            "François",
                            "Girard",
                            "francois.girard@email.com",
                            "+33602030405",
                            "789 Boulevard Haussmann, Paris"));

            parties.add(
                    createPartie(
                            org001Dossiers.get(2),
                            PartiePrenanteRole.SELLER,
                            "Pierre",
                            "Durand",
                            "pierre.durand@email.com",
                            "+33623456789",
                            "321 Rue du Commerce, Marseille"));

            parties.add(
                    createPartie(
                            org001Dossiers.get(2),
                            PartiePrenanteRole.NOTARY,
                            "Isabelle",
                            "Laurent",
                            "isabelle.laurent@notaire.fr",
                            "+33603040506",
                            "Notary Office, Marseille"));
        }
        ...
        if (org002Dossiers.size() >= 3) {
            parties.add(
                    createPartie(
                            org002Dossiers.get(0),
                            PartiePrenanteRole.BUYER,
                            "Alice",
                            "Rousseau",
                            "alice.rousseau@email.com",
                            "+33711223344",
            ...
            parties.add(
                    createPartie(
                            ...
                            "Marc",
                            "Dubois",
                            "marc.dubois@realestate.com",
                            "+33701112233",
                            "Real Estate Office, Paris"));

            parties.add(
                    createPartie(
                            org002Dossiers.get(1),
                            PartiePrenanteRole.TENANT,
                            "Bob",
                            "Lefebvre",
                            "bob.lefebvre@email.com",
                            "+33722334455",
                            "88 Avenue des Champs, Lyon"));

            parties.add(
                    createPartie(
                            org002Dossiers.get(2),
                            PartiePrenanteRole.LANDLORD,
                            "Catherine",
                            "Blanc",
                            "catherine.blanc@email.com",
                            "+33733445566",
        ...
        logger.info(
                "Loaded {} parties prenantes ({} for ORG-001, {} for ORG-002)",
                savedParties.size(),
                6,
                4);
        logger.info(
                "Note: Duplicate phone entry exists - dossier #1 and #10 share phone +33612345678 (ORG-001)");
    }

    private PartiePrenanteEntity createPartie(
            Dossier dossier,
            PartiePrenanteRole role,
            String firstName,
            String lastName,
            String email,
            String phone,
