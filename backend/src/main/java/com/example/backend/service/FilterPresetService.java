...
public class FilterPresetService {
    ...
    public FilterPresetService(
            FilterPresetRepository filterPresetRepository, FilterPresetMapper filterPresetMapper) {
    ...
    public FilterPresetResponse getById(Long id) {
        FilterPresetEntity entity =
                filterPresetRepository
                        .findById(id)
                        .orElseThrow(
                                () ->
                                        new EntityNotFoundException(
                                                "Filter preset not found with id: " + id));
    ...
    public List<FilterPresetResponse> getAccessiblePresets(String filterType) {
        ...
        List<FilterPresetEntity> presets =
                filterPresetRepository.findAccessibleByFilterTypeAndOrgId(
                        filterType, orgId, userId);

        return presets.stream().map(filterPresetMapper::toResponse).collect(Collectors.toList());
    }
    ...
    public List<FilterPresetResponse> getPredefinedPresets(String filterType) {
        ...
        List<FilterPresetEntity> presets =
                filterPresetRepository.findPredefinedByFilterType(filterType, orgId);
    ...
    public List<FilterPresetResponse> getUserPresets(String filterType) {
        ...
        List<FilterPresetEntity> presets =
                filterPresetRepository.findByFilterTypeAndCreatedBy(filterType, userId, orgId);
