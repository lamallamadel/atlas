...
public class MetabaseDashboardService {
    ...
    public MetabaseEmbedResponse generateEmbedUrl(MetabaseEmbedRequest request) {
        ...
        Long expirationMinutes = request.getExpirationMinutes() != null ? request.getExpirationMinutes() : 10L;
        ...
        String embedUrl = String.format("%s/embed/dashboard/%s#bordered=false&titled=false", 
                                        metabaseUrl, token);
    ...
    public MetabaseEmbedResponse generateQuestionEmbedUrl(String questionId, Map<String, Object> params, Long expirationMinutes) {
        ...
        String token = generateJwtToken(payload, expiration);
        String embedUrl = String.format("%s/embed/question/%s#bordered=false&titled=false", 
                                        metabaseUrl, token);
    ...
    public void createDashboard(String dashboardName, String description, Map<String, Object> metadata) {
        try {
            ...
            webClient.post()
                ...
                .doOnSuccess(response -> 
                    logger.info("Dashboard created successfully: {}", response.get("id")))
                .doOnError(error -> 
                    logger.error("Failed to create dashboard: {}", error.getMessage()))
                .subscribe();
