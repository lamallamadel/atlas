...
public class DossierStatusTransitionService {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(DossierStatusTransitionService.class);
    ...
    public DossierStatusTransitionService(
    ...
    /**
     * Initializes basic status transition rules that apply to all dossiers. These rules are always
     * enforced, regardless of whether caseType is set.
     *
     * <p>When caseType is null, ONLY these rules apply (workflow validation bypassed). When
     * caseType is set, these rules AND custom workflow rules both apply.
     *
     * <p>Transition Map: - NEW: Can go to QUALIFYING (start qualification), QUALIFIED
     * (pre-qualified client), APPOINTMENT (direct scheduling), or LOST (client not interested) -
     ...
     * APPOINTMENT: Can go to WON (deal closed) or LOST (client rejected offer) - WON: Terminal
     * state - no transitions allowed - LOST: Terminal state - no transitions allowed
     */
    ...
    public void recordTransition(
        ...
        metricsService.incrementDossierStatusTransition(
                fromStatus != null ? fromStatus.name() : "NONE", toStatus.name());

        logStatusChangeActivity(dossier, fromStatus, toStatus, userId, reason);
    }

    private void logStatusChangeActivity(
            Dossier dossier,
            DossierStatus fromStatus,
            DossierStatus toStatus,
            String userId,
        ...
        if (activityService != null) {
            try {
                String description =
                        String.format("Status changed from %s to %s", fromStatus, toStatus);
                ...
                activityService.logActivity(
                        dossier.getId(), ActivityType.STATUS_CHANGE, description, metadata);
            } catch (Exception e) {
                logger.warn(
                        "Failed to log status change activity for dossier {}: {}",
                        dossier.getId(),
                        e.getMessage(),
                        e);
            }
