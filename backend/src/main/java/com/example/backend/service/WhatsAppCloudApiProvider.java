...
public class WhatsAppCloudApiProvider implements OutboundMessageProvider {
    ...
    public ProviderSendResult send(OutboundMessageEntity message) {
        try {
            ...
            if (!rateLimitService.checkAndConsumeQuota(message.getOrgId())) {
                return ProviderSendResult.failure("QUOTA_EXCEEDED", "WhatsApp quota exceeded or rate limited", true, null);
            }
            
            boolean withinSessionWindow = sessionWindowService.isWithinSessionWindow(message.getOrgId(), message.getTo());
            boolean hasTemplate = message.getTemplateCode() != null && !message.getTemplateCode().isEmpty();
            
            if (!withinSessionWindow && !hasTemplate) {
                logger.warn("Message outside 24h window without template: messageId={}, to={}", 
            ...
            logger.info("Sending WhatsApp message: orgId={}, messageId={}, to={}, template={}, withinWindow={}", 
                message.getOrgId(), message.getId(), message.getTo(), message.getTemplateCode(), withinSessionWindow);
    ...
    private Map<String, Object> buildPayload(OutboundMessageEntity message, WhatsAppProviderConfig config, boolean withinSessionWindow) {
        Map<String, Object> payload = new HashMap<>();
        payload.put("messaging_product", "whatsapp");
        payload.put("recipient_type", "individual");
        payload.put("to", normalizePhoneNumber(message.getTo()));
        
        boolean useTemplate = message.getTemplateCode() != null && !message.getTemplateCode().isEmpty();
        
        if (!withinSessionWindow && !useTemplate) {
            logger.warn("Forcing template usage for message outside session window: messageId={}", message.getId());
            useTemplate = true;
        }
    ...
    private ProviderSendResult handleHttpError(HttpStatusCodeException e, OutboundMessageEntity message) {
        ...
        logger.error("WhatsApp API error response for messageId={}: status={}, body={}", 
        ...
        try {
            ...
            if (errorCode != null) {
                ...
                logger.warn("WhatsApp error for messageId={}: code={}, message={}, retryable={}", 
            ...
            logger.error("Failed to parse error response for messageId={}", message.getId(), parseEx);
            return ProviderSendResult.failure(
                "HTTP_" + e.getStatusCode().value(), 
    ...
    private String sanitizeErrorMessage(String message) {
        ...
        message = message.replaceAll("(?i)(api[_-]?key|secret|token|password|bearer)\\s*[:=]\\s*[^\\s,}]+", "$1=***");
