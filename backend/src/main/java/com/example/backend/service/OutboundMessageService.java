...
public class OutboundMessageService {
    ...
    public OutboundMessageEntity createOutboundMessage(
        ...
        Optional<OutboundMessageEntity> existing =
                outboundMessageRepository.findByOrgIdAndIdempotencyKey(orgId, idempotencyKey);

        if (existing.isPresent()) {
            logger.info(
                    "Returning existing outbound message for idempotency key: {}", idempotencyKey);
            return existing.get();
        }
        ...
        logger.info(
                "Created outbound message: id={}, orgId={}, dossierId={}, channel={}, to={}",
                saved.getId(),
                orgId,
                dossierId,
                channel,
        ...
        return switch (channel) {
            ...
            default ->
                    throw new IllegalArgumentException(
                            "Unsupported message channel for consent validation: " + channel);
        };
    }
        ...
        OutboundMessageEntity message =
                outboundMessageRepository
                        ...
                        .orElseThrow(
                                () ->
                                        new EntityNotFoundException(
                                                "Outbound message not found with id: " + id));
    ...
    @Transactional
    public OutboundMessageEntity updateStatus(
            Long id, OutboundMessageStatus status, String errorCode, String errorMessage) {
        OutboundMessageEntity message =
                outboundMessageRepository
                        .findById(id)
                        .orElseThrow(
