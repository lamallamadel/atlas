...
public class JavaMailEmailProvider implements OutboundMessageProvider {
    ...
    public ProviderSendResult send(OutboundMessageEntity message) {
        try {
            ...
            mimeMessage.setHeader(
                    "Message-ID",
                    "<" + messageId + "@" + extractDomain(config.getFromEmail()) + ">");
            ...
            logger.info(
                    "Email sent successfully: messageId={}, providerMessageId={}",
                    message.getId(),
                    messageId);
            ...
            logger.error(
                    "SMTP authentication failed for messageId={}: {}",
                    message.getId(),
                    e.getMessage());
            return ProviderSendResult.failure(
                    "AUTH_FAILED", "SMTP authentication failed", false, null);
                ...
                new jakarta.mail.Authenticator() {
                    protected jakarta.mail.PasswordAuthentication getPasswordAuthentication() {
                        return new jakarta.mail.PasswordAuthentication(
                                config.getSmtpUsername(), config.getSmtpPasswordEncrypted());
                    }
                });
    }

    private MimeMessage buildMimeMessage(
            Session session, OutboundMessageEntity message, EmailProviderConfig config)
    ...
    private void buildMessageContent(MimeMessage mimeMessage, Map<String, Object> payload)
    ...
    private void validateEmailMessage(OutboundMessageEntity message) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Invalid recipient email address: " + message.getTo(), e);
        }
        ...
        if (payload != null) {
            ...
            if (estimatedSize > MAX_MESSAGE_SIZE_MB * 1024 * 1024) {
                throw new IllegalArgumentException(
                        "Message size exceeds limit of " + MAX_MESSAGE_SIZE_MB + "MB");
            }
        }
    }

    private ProviderSendResult handleSendFailedException(
            SendFailedException e, OutboundMessageEntity message) {
        ...
        if (e.getInvalidAddresses() != null && e.getInvalidAddresses().length > 0) {
            return ProviderSendResult.failure(
                    "INVALID_EMAIL", "Invalid recipient email address", false, null);
        }
    ...
    private ProviderSendResult handleMessagingException(
        ...
        if (cause instanceof ConnectException) {
            return ProviderSendResult.failure(
                    "CONNECTION_FAILED", "Could not connect to SMTP server", true, null);
        }
        ...
        if (errorMessage != null && errorMessage.toLowerCase().contains("tls")) {
            return ProviderSendResult.failure(
                    "TLS_FAILED", "TLS/SSL negotiation failed", true, null);
        }
