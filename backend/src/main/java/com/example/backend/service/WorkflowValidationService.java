 ...
 *
 * <p>TWO-TIER VALIDATION ARCHITECTURE:
 *
 * <p>1. BASIC VALIDATION (DossierStatusTransitionService) - ALWAYS APPLIED - Enforces terminal
 * states (WON, LOST cannot transition) - Validates allowed transitions (e.g., NEW can go to
 * QUALIFYING, QUALIFIED, APPOINTMENT, LOST) - Applied to ALL dossiers regardless of caseType
 *
 * <p>2. WORKFLOW VALIDATION (This Service) - CONDITIONAL - Only applied when dossier.caseType is
 * set (not null/blank) - Enforces custom rules per case type (e.g., "SALE", "RENTAL") - Can require
 * specific fields, validate conditions, or impose stricter transitions - Records all transition
 * attempts in workflow_transition table for audit - Enforces role-based authorization for
 * transitions - Validates pre-conditions (e.g., appointments) before allowing transitions
 *
 * <p>BYPASS MECHANISM: When caseType is null or blank, this validation is skipped, allowing: -
 * Generic dossiers without workflow constraints - Legacy data migration without workflow setup -
...
public class WorkflowValidationService {
    ...
    @Transactional
    public void validateAndRecordTransition(
            Dossier dossier, String fromStatus, String toStatus, String userId, String reason) {
        ...
        Optional<WorkflowDefinition> workflowDef =
                workflowDefinitionRepository.findActiveTransition(
        ...
        if (workflowDef.isEmpty()) {
            ...
            errors.put(
                    "transition",
                    String.format(
                            "Transition from %s to %s is not allowed for case type %s",
            ...
            errors.put(
                    ...
                    String.format(
                            ...
                            fromStatus,
                            toStatus,
                            String.join(", ", getAllowedTransitions(orgId, caseType, fromStatus))));
            ...
            throw new WorkflowValidationException(
                    String.format(
                            "Invalid workflow transition from %s to %s for case type %s",
                            fromStatus, toStatus, caseType),
                    errors);
        }
        ...
        if (!validationErrors.isEmpty()) {
            ...
            throw new WorkflowValidationException(
                    String.format(
                            "Workflow validation failed for transition from %s to %s",
                            fromStatus, toStatus),
                    validationErrors);
        }
    ...
    private void validateRequiredFields(
        ...
        boolean isTerminalState =
                "LOST".equalsIgnoreCase(toStatus) || "WON".equalsIgnoreCase(toStatus);
        ...
        if (isTerminalState && "LOST".equalsIgnoreCase(toStatus)) {
            if (dossier.getLossReason() == null || dossier.getLossReason().isBlank()) {
                ...
                actionableMessages.add(
                        "Loss reason is required when marking a dossier as LOST. "
        ...
        if (isTerminalState && "WON".equalsIgnoreCase(toStatus)) {
            if (dossier.getWonReason() == null || dossier.getWonReason().isBlank()) {
                ...
                actionableMessages.add(
                        "Won reason is required when marking a dossier as WON. "
        ...
        if ("CRM_QUALIFIED".equalsIgnoreCase(toStatus)) {
            ...
            boolean hasAgentRole =
                    authorities.stream()
                            .anyMatch(
                                    auth ->
                                            auth.getAuthority().equalsIgnoreCase("ROLE_AGENT")
                                                    || auth.getAuthority()
            ...
            if (!hasAgentRole) {
                ...
                errors.put(
                        "roleActionableMessage",
                        String.format(
                                "Only users with the AGENT role can move dossiers to '%s' status. "
        ...
        if ("CRM_VISIT_DONE".equalsIgnoreCase(toStatus)) {
            ...
            if (!hasCompletedAppointment) {
                ...
                errors.put(
                        ...
                        String.format(
                                "Before moving to '%s' status, you must have at least one completed appointment. "
        ...
        if (conditions != null && conditions.containsKey("requiresAppointment")) {
            ...
            if (Boolean.TRUE.equals(requiresAppointment)) {
                ...
                if (appointments == null || appointments.isEmpty()) {
                    errors.put(
                            "preConditionError",
                            "At least one appointment is required for this transition");
                    errors.put(
                            "preConditionActionableMessage",
    ...
    private void validateCustomConditions(
            Dossier dossier, WorkflowDefinition definition, Map<String, Object> errors) {
        Map<String, Object> conditions = definition.getConditionsJson();
                ...
                if (field != null && operator != null) {
                    ...
                    if (!evaluateCondition(actualValue, operator, expectedValue)) {
                        ...
                        String actionableMessage =
                                buildActionableMessageForCondition(
    ...
    private String buildActionableMessageForCondition(
            String field, String operator, Object expectedValue, Object actualValue) {
        String actualValueStr = actualValue == null ? "not set" : actualValue.toString();

        switch (operator) {
            case "equals":
                return String.format(
                        "Field '%s' must equal '%s', but current value is '%s'. "
                                + "Please update this field to the required value.",
                        field, expectedValue, actualValueStr);
            case "notEquals":
                return String.format(
                        "Field '%s' must not equal '%s', but current value is '%s'. "
                                + "Please change this field to a different value.",
                        field, expectedValue, actualValueStr);
            case "greaterThan":
                return String.format(
                        "Field '%s' must be greater than '%s', but current value is '%s'. "
                                + "Please increase the value of this field.",
                        field, expectedValue, actualValueStr);
            case "lessThan":
                return String.format(
                        "Field '%s' must be less than '%s', but current value is '%s'. "
                                + "Please decrease the value of this field.",
                        field, expectedValue, actualValueStr);
            case "greaterThanOrEqual":
                return String.format(
                        "Field '%s' must be greater than or equal to '%s', but current value is '%s'. "
                                + "Please ensure the value meets the minimum requirement.",
                        field, expectedValue, actualValueStr);
            case "lessThanOrEqual":
                return String.format(
                        "Field '%s' must be less than or equal to '%s', but current value is '%s'. "
                                + "Please ensure the value does not exceed the maximum.",
                        field, expectedValue, actualValueStr);
            case "isNull":
                return String.format(
                        "Field '%s' must be empty/null, but current value is '%s'. "
                                + "Please clear this field.",
                        field, actualValueStr);
            case "isNotNull":
                return String.format(
                        "Field '%s' must have a value, but it is currently empty. "
                                + "Please provide a value for this field.",
                        field);
            case "isEmpty":
                return String.format(
                        "Field '%s' must be empty, but current value is '%s'. "
                                + "Please clear this field.",
                        field, actualValueStr);
            case "isNotEmpty":
                return String.format(
            ...
            default:
                return String.format(
                        "Field '%s' does not meet the condition: %s %s. Current value: '%s'",
    ...
    private List<String> getAllowedTransitions(String orgId, String caseType, String fromStatus) {
        List<WorkflowDefinition> allowed =
                workflowDefinitionRepository.findAllowedTransitionsFrom(
                        orgId, caseType, fromStatus);
        return allowed.stream().map(WorkflowDefinition::getToStatus).toList();
    }
    ...
    private boolean evaluateCondition(Object actualValue, String operator, Object expectedValue) {
        switch (operator) {
            ...
            case "isEmpty":
                return actualValue == null
                        || (actualValue instanceof String && ((String) actualValue).isEmpty());
            case "isNotEmpty":
                return actualValue != null
                        && !(actualValue instanceof String && ((String) actualValue).isEmpty());
            default:
    ...
    public Map<String, Object> checkTransitionValidity(
            Dossier dossier, String fromStatus, String toStatus) {
        String orgId = TenantContext.getOrgId();
        if (orgId == null) {
            throw new IllegalStateException("Organization ID not found in context");
        }
