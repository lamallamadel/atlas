...
public class WebhookRetryWorker {
    ...
    public WebhookRetryWorker(WebhookDeliveryRepository deliveryRepository,
    ...
    public void processRetries() {
        ...
        List<WebhookDeliveryEntity> pendingRetries = deliveryRepository.findByStatusAndNextRetryAtBefore(
            WebhookDeliveryEntity.DeliveryStatus.RETRY,
            LocalDateTime.now()
        );

        for (WebhookDeliveryEntity delivery : pendingRetries) {
            subscriptionRepository.findById(delivery.getSubscriptionId()).ifPresent(subscription -> {
                logger.info("Retrying webhook delivery {} for subscription {}", 
                    delivery.getId(), subscription.getId());
