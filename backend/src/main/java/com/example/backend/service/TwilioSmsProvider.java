...
public class TwilioSmsProvider implements OutboundMessageProvider {
    ...
    private ProviderSendResult sendInternal(OutboundMessageEntity message) {
        try {
            ...
            if (!"TWILIO".equalsIgnoreCase(config.getProviderType())) {
                return ProviderSendResult.failure(
                        "INVALID_PROVIDER", "Provider type is not TWILIO", false, null);
            }
            ...
            String url =
                    String.format(
                            "%s/Accounts/%s/Messages.json", baseUrl, config.getTwilioAccountSid());

            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
            headers.setBasicAuth(
                    config.getTwilioAccountSid(), config.getTwilioAuthTokenEncrypted());
            ...
            logger.info(
                    "Sending SMS via Twilio: orgId={}, messageId={}, to={}",
                    message.getOrgId(),
                    message.getId(),
                    message.getTo());
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                logger.info(
                        "SMS sent via Twilio successfully: messageId={}, providerMessageId={}",
                        message.getId(),
                        providerMessageId);
    ...
    private ProviderSendResult handleHttpError(
        ...
        logger.error(
                "Twilio API error response for messageId={}: status={}, body={}",
                message.getId(),
                e.getStatusCode(),
                responseBody);
        ...
        try {
            ...
            if (errorCodeStr != null) {
                ...
                logger.warn(
                        "Twilio error for messageId={}: code={}, message={}, retryable={}",
                        message.getId(),
                        errorCodeStr,
                        errorMessage,
            ...
            logger.error(
                    "Failed to parse Twilio error response for messageId={}",
                    message.getId(),
                    parseEx);
            return ProviderSendResult.failure(
                    "HTTP_" + e.getStatusCode().value(),
            ...
            if (message != null && message.contains("\"code\"")) {
                ...
                if (codeStart != -1) {
                    ...
                    if (codeValueStart > 0 && codeValueEnd > codeValueStart) {
                        String code =
                                message.substring(codeValueStart, codeValueEnd)
                                        .trim()
                                        .replaceAll("[^0-9]", "");
                        return "TWILIO_" + code;
                    }
