...
public class CustomDomainService {
    ...
    public CustomDomainService(
            CustomDomainMappingRepository domainRepository, WhiteLabelService whiteLabelService) {
        this.domainRepository = domainRepository;
        this.whiteLabelService = whiteLabelService;
    }

    @Transactional
    public CustomDomainMappingEntity addCustomDomain(
            String orgId, String domain, boolean isPrimary) {
        ...
        if (isPrimary) {
            domainRepository
                    .findByOrgIdAndIsPrimary(orgId, true)
                    .ifPresent(
                            primary -> {
    ...
    public boolean verifyDnsConfiguration(String domain) {
        ...
        CustomDomainMappingEntity mapping =
                domainRepository
                        .findByDomain(domain)
                        .orElseThrow(
                                () -> new IllegalStateException("Domain not found: " + domain));
        ...
        boolean dnsVerified =
                performDnsVerification(
                        domain, mapping.getCnameTarget(), mapping.getTxtVerificationCode());
    ...
    private void provisionSslCertificate(CustomDomainMappingEntity mapping) {
        ...
        try {
            ...
            logger.info(
                    "SSL certificate provisioned successfully for domain: {}", mapping.getDomain());

        } catch (Exception e) {
            logger.error(
                    "Failed to provision SSL certificate for domain: {}", mapping.getDomain(), e);
