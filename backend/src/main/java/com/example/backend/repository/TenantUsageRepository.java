...
public interface TenantUsageRepository extends JpaRepository<TenantUsageEntity, Long> {
    ...
    @Query("SELECT u FROM TenantUsageEntity u WHERE u.orgId = :orgId AND u.periodStart >= :startDate AND u.periodEnd <= :endDate")
    List<TenantUsageEntity> findUsageInPeriod(String orgId, LocalDateTime startDate, LocalDateTime endDate);
}
