...
public interface WorkflowTransitionRepository extends JpaRepository<WorkflowTransition, Long> {
    ...
    @Query(
            "SELECT wt FROM WorkflowTransition wt WHERE wt.dossierId = :dossierId AND wt.isAllowed = false")
    List<WorkflowTransition> findFailedTransitionsByDossierId(@Param("dossierId") Long dossierId);

    @Query(
            "SELECT wt FROM WorkflowTransition wt WHERE wt.orgId = :orgId AND wt.transitionedAt >= :startDate")
    ...
    @Query(
            "SELECT COUNT(wt) FROM WorkflowTransition wt WHERE wt.orgId = :orgId AND wt.isAllowed = false AND wt.transitionedAt >= :startDate")
