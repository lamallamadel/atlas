...
public interface TaskRepository
    ...
    @Query(
            "SELECT t FROM TaskEntity t WHERE t.assignedTo = :assignedTo "
    ...
    @Query(
            "SELECT t FROM TaskEntity t WHERE t.orgId = :orgId "
                    + "AND t.status IN ('TODO', 'IN_PROGRESS') "
                    + "AND t.dueDate IS NOT NULL "
                    + "AND t.dueDate < :currentDate "
                    + "ORDER BY t.dueDate ASC")
    List<TaskEntity> findAllOverdueTasksForOrg(
            @Param("orgId") String orgId, @Param("currentDate") LocalDateTime currentDate);
}
