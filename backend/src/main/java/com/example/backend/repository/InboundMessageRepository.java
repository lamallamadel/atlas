...
public interface InboundMessageRepository extends JpaRepository<InboundMessageEntity, Long> {
    ...
    @Query(
            "SELECT i FROM InboundMessageEntity i WHERE i.orgId = :orgId AND i.phoneNumber = :phoneNumber ORDER BY i.receivedAt DESC")
    ...
    @Query(
            "SELECT i FROM InboundMessageEntity i WHERE i.orgId = :orgId AND i.processedAt IS NULL ORDER BY i.receivedAt ASC")
    ...
    @Query(
            "SELECT i FROM InboundMessageEntity i WHERE i.orgId = :orgId AND i.phoneNumber = :phoneNumber AND i.receivedAt >= :since ORDER BY i.receivedAt DESC")
