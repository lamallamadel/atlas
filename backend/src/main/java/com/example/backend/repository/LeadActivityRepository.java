...
public interface LeadActivityRepository extends JpaRepository<LeadActivity, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(a) FROM LeadActivity a JOIN a.dossier d WHERE d.annonceId = :annonceId AND a.createdAt >= :startDate")
