...
public interface FilterPresetRepository extends JpaRepository<FilterPresetEntity, Long> {
    ...
    @Query(
            "SELECT f FROM FilterPresetEntity f WHERE f.filterType = :filterType AND f.orgId = :orgId AND (f.isShared = true OR f.createdBy = :userId)")
    ...
    @Query(
            "SELECT f FROM FilterPresetEntity f WHERE f.filterType = :filterType AND f.orgId = :orgId AND f.isPredefined = true")
    List<FilterPresetEntity> findPredefinedByFilterType(
            @Param("filterType") String filterType, @Param("orgId") String orgId);

    @Query(
            "SELECT f FROM FilterPresetEntity f WHERE f.filterType = :filterType AND f.createdBy = :userId AND f.orgId = :orgId")
