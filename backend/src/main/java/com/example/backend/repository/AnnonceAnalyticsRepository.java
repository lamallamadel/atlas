...
public interface AnnonceAnalyticsRepository extends JpaRepository<Annonce, Long> {
    ...
    @Query(
            "SELECT COALESCE(SUM(a.price), 0) FROM Annonce a "
                    + "WHERE a.orgId = :orgId "
    ...
    @Query(
            "SELECT COALESCE(SUM(a.price), 0) FROM Annonce a "
                    ...
                    + "AND a.price IS NOT NULL")
    BigDecimal getPipelineValueByStatuses(
            @Param("orgId") String orgId, @Param("statuses") List<String> statuses);

    @Query(
            "SELECT d.source as source, COALESCE(SUM(a.price), 0) as totalValue, COUNT(d) as count "
                    + "FROM Annonce a "
                    + "JOIN Dossier d ON d.annonceId = a.id "
                    + "WHERE a.orgId = :orgId "
                    + "AND d.orgId = :orgId "
    ...
    @Query(
            "SELECT COUNT(DISTINCT d) FROM Dossier d "
    ...
    @Query(
            "SELECT COALESCE(AVG(a.price), 0) FROM Annonce a "
                    + "JOIN Dossier d ON d.annonceId = a.id "
                    + "WHERE a.orgId = :orgId "
                    + "AND d.orgId = :orgId "
                    + "AND d.status = 'WON' "
    ...
    BigDecimal getAverageWonDealValue(
    ...
    @Query(
            "SELECT COUNT(d) FROM Dossier d "
                    ...
                    + "AND d.createdAt >= :startDate "
                    + "AND d.createdAt < :endDate")
    Long getWonDossiersInPeriod(
            @Param("orgId") String orgId,
            @Param("startDate") LocalDateTime startDate,
            @Param("endDate") LocalDateTime endDate);

    @Query(
            "SELECT d.source as source, COUNT(d) as total, "
                    ...
                    + "AND d.createdAt >= :startDate "
                    + "GROUP BY d.source")
    List<Object[]> getConversionRatesBySource(
            @Param("orgId") String orgId, @Param("startDate") LocalDateTime startDate);

    @Query(
            "SELECT d.status as status, COUNT(d) as count, COALESCE(SUM(a.price), 0) as totalValue "
                    + "FROM Dossier d "
