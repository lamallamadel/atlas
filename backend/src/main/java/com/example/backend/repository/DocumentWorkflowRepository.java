...
public interface DocumentWorkflowRepository extends JpaRepository<DocumentWorkflowEntity, Long> {
    ...
    @Query(
            "SELECT w FROM DocumentWorkflowEntity w WHERE w.orgId = :orgId AND w.status = :status AND w.currentStepOrder IS NOT NULL")
    List<DocumentWorkflowEntity> findActiveWorkflowsByStatus(
            @Param("orgId") String orgId, @Param("status") WorkflowStatus status);

    @Query(
            "SELECT w FROM DocumentWorkflowEntity w WHERE w.orgId = :orgId AND w.initiatedBy = :userId ORDER BY w.createdAt DESC")
    List<DocumentWorkflowEntity> findByInitiatedBy(
