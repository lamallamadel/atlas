...
public interface UserPreferencesRepository extends JpaRepository<UserPreferencesEntity, Long> {
    ...
    Optional<UserPreferencesEntity> findByUserIdAndOrgId(@Param("userId") String userId, @Param("orgId") String orgId);

    @Query("SELECT COUNT(u) > 0 FROM UserPreferencesEntity u WHERE u.userId = :userId AND u.orgId = :orgId")
