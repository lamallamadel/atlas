...
public interface WorkflowApprovalRepository extends JpaRepository<WorkflowApprovalEntity, Long> {
    ...
    Optional<WorkflowApprovalEntity> findByStepIdAndApproverIdAndOrgId(
            Long stepId, String approverId, String orgId);

    List<WorkflowApprovalEntity> findByApproverIdAndDecisionIsNullAndOrgId(
            String approverId, String orgId);

    @Query(
            "SELECT a FROM WorkflowApprovalEntity a WHERE a.stepId = :stepId AND a.orgId = :orgId AND a.decision = :decision")
    List<WorkflowApprovalEntity> findApprovalsByStepAndDecision(
            @Param("stepId") Long stepId,
            @Param("orgId") String orgId,
            @Param("decision") WorkflowStepStatus decision);
}
