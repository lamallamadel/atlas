...
public interface WorkflowTemplateRepository extends JpaRepository<WorkflowTemplateEntity, Long> {
    ...
    List<WorkflowTemplateEntity> findByWorkflowTypeAndOrgIdAndIsActiveTrue(
            DocumentWorkflowType workflowType, String orgId);

    List<WorkflowTemplateEntity> findByIsSystemTemplateTrueAndIsActiveTrue();

    @Query(
            "SELECT t FROM WorkflowTemplateEntity t WHERE t.orgId = :orgId AND t.isActive = true ORDER BY t.usageCount DESC")
