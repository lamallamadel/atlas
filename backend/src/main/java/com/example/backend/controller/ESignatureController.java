...
public class ESignatureController {
    ...
    @Operation(summary = "Create a signature request", description = "Creates a new signature request for a dossier")
    public ResponseEntity<SignatureRequestResponse> createSignatureRequest(
        ...
        try {
            SignatureRequestEntity entity = eSignatureService.createSignatureRequest(request, orgId, userId);
            return ResponseEntity.status(HttpStatus.CREATED).body(signatureRequestMapper.toResponse(entity));
        } catch (Exception e) {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(summary = "Send signature request", description = "Sends the signature request to DocuSign")
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(summary = "Get signature requests by dossier", description = "Retrieves all signature requests for a dossier")
    public ResponseEntity<List<SignatureRequestResponse>> getSignatureRequestsByDossier(
            @PathVariable Long dossierId,
            @RequestHeader("X-Org-Id") String orgId) {
        List<SignatureRequestEntity> entities = eSignatureService.getSignatureRequestsByDossier(dossierId, orgId);
        List<SignatureRequestResponse> responses = entities.stream()
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(summary = "Get signature request by ID", description = "Retrieves a signature request by its ID")
    ...
    @PostMapping("/webhook")
    @Operation(summary = "DocuSign webhook callback", description = "Receives webhook events from DocuSign")
    public ResponseEntity<Map<String, String>> handleDocuSignWebhook(@RequestBody String payload) {
        try {
            Map<String, Object> webhookData = new com.fasterxml.jackson.databind.ObjectMapper().readValue(payload, Map.class);
    ...
    @Operation(summary = "Upload contract template", description = "Uploads a PDF contract template")
    public ResponseEntity<ContractTemplateResponse> uploadTemplate(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body(contractTemplateMapper.toResponse(entity));
        } catch (Exception e) {
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(summary = "Get active templates", description = "Retrieves all active contract templates")
    public ResponseEntity<List<ContractTemplateResponse>> getActiveTemplates(
            @RequestHeader("X-Org-Id") String orgId) {
        List<ContractTemplateResponse> responses = contractTemplateService.getActiveTemplates(orgId).stream()
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(summary = "Get template by ID", description = "Retrieves a contract template by its ID")
