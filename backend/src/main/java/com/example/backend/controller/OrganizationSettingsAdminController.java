...
@Tag(
        ...
        description = "Admin API for managing organization settings")
public class OrganizationSettingsAdminController {

    private static final Logger logger =
            LoggerFactory.getLogger(OrganizationSettingsAdminController.class);

    private final OrganizationSettingsService organizationSettingsService;

    public OrganizationSettingsAdminController(
    ...
    @Operation(
            ...
            description =
                    "Updates settings for the current organization (Admin only). Validates that orgId matches tenant context.")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                ...
                @ApiResponse(
                        responseCode = "400",
                        description = "Invalid request data or tenant context mismatch"),
                @ApiResponse(
                        responseCode = "403",
    ...
    @Operation(
            ...
            description =
                    "Retrieves branding settings for the current organization for theming purposes. Publicly accessible.")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(
            summary = "Test integration provider connection",
            description =
                    "Tests connection to an integration provider before saving configuration (Admin only)")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<IntegrationTestResponse> testIntegrationConnection(
        ...
        logger.info(
                "Testing integration connection for organization: {}, provider: {}",
                orgId,
                request.getProviderType());

        IntegrationTestResponse response =
                organizationSettingsService.testIntegrationConnection(request);
        return ResponseEntity.ok(response);
    }
}
