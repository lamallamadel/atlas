...
public class TaskController {
    ...
    @Operation(
            ...
            description = "Creates a new task with optional assignment and due date")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "201",
    ...
    public ResponseEntity<TaskResponse> update(
            @Parameter(description = "ID of the task to update", required = true) @PathVariable
                    Long id,
    ...
    @Operation(
            ...
            description = "Marks a task as completed with optional completion notes")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<TaskResponse> complete(
            @Parameter(description = "ID of the task to complete", required = true) @PathVariable
                    Long id,
    ...
    public ResponseEntity<Void> delete(
            @Parameter(description = "ID of the task to delete", required = true) @PathVariable
                    Long id) {
        try {
    ...
    @Operation(
            summary = "List tasks",
            description =
                    "Retrieves a paginated list of tasks with optional filtering by dossier, assignee, status, due date range, and overdue status")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<Page<TaskResponse>> list(
            @Parameter(description = "Filter by dossier ID") @RequestParam(required = false)
                    Long dossierId,
            @Parameter(description = "Filter by assigned user") @RequestParam(required = false)
                    String assignedTo,
            @Parameter(description = "Filter by task status") @RequestParam(required = false)
                    TaskStatus status,
            @Parameter(
                            description =
                                    "Filter by due date before (ISO 8601 format: yyyy-MM-dd'T'HH:mm:ss)")
                    @RequestParam(required = false)
                    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)
                    LocalDateTime dueBefore,
            @Parameter(
                            description =
                                    "Filter by due date after (ISO 8601 format: yyyy-MM-dd'T'HH:mm:ss)")
                    @RequestParam(required = false)
                    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)
                    LocalDateTime dueAfter,
            @Parameter(description = "Filter overdue tasks only") @RequestParam(required = false)
                    Boolean overdue,
            ...
            @Parameter(
                    ...
                    @RequestParam(defaultValue = "dueDate,asc")
                    String sort) {
