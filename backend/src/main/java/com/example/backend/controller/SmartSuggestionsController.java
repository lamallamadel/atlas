...
@Tag(
        ...
        description = "Intelligent suggestions and contextual recommendations")
public class SmartSuggestionsController {
    ...
    @Operation(
            summary = "Get smart suggestions for a dossier",
            description =
                    "Returns contextual next-best-action suggestions based on dossier status, activity, and user behavior")
    public ResponseEntity<List<SmartSuggestionResponse>> getSuggestionsForDossier(
        ...
        List<SmartSuggestionResponse> suggestions =
                smartSuggestionsService.getSuggestionsForDossier(dossierId);
    ...
    @Operation(
            ...
            description =
                    "Records user actions to improve future suggestions through machine learning")
    public ResponseEntity<Void> trackBehavior(@Valid @RequestBody TrackBehaviorRequest request) {
        smartSuggestionsService.trackBehavior(
                request.getActionType(), request.getContextType(), request.getContextId());
    ...
    @Operation(
            ...
            description =
                    "Records whether a suggestion was accepted or rejected to improve ML model")
    ...
    public ResponseEntity<List<MessageTemplateResponse>> getMessageTemplates(
        ...
        List<MessageTemplateResponse> templates =
                smartSuggestionsService.getMessageTemplates(category, channel);
    ...
    public ResponseEntity<PrefilledMessageResponse> getPrefilledMessage(
            ...
            @Parameter(description = "Dossier ID for context", required = true) @RequestParam
                    Long dossierId) {
        PrefilledMessageResponse message =
                smartSuggestionsService.getPrefilledMessage(templateId, dossierId);
    ...
    public ResponseEntity<Void> recordTemplateUsage(
            @Parameter(description = "Template ID", required = true) @PathVariable
                    Long templateId) {
