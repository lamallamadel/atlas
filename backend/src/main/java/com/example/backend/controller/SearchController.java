...
public class SearchController {
    ...
    @GetMapping
    @Operation(
            summary = "Search across Annonces and Dossiers",
            description =
                    "Performs full-text search with fuzzy matching and relevance scoring. Falls back to PostgreSQL when Elasticsearch is unavailable.")
    public ResponseEntity<SearchResponseDto> search(
            @Parameter(description = "Search query string") @RequestParam(required = false)
                    String q,
            ...
            @Parameter(
                            description =
                                    "JSON-encoded filters object. Example: {\"status\":\"ACTIVE\",\"city\":\"Paris\"}")
                    @RequestParam(required = false)
                    String filters,
            ...
            try {
                filterMap =
                        objectMapper.readValue(
                                filters, new TypeReference<Map<String, Object>>() {});
            } catch (Exception e) {
