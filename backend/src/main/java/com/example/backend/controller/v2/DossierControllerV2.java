...
@Tag(name = "Dossiers V2", description = "API v2 for managing dossiers with enhanced response format")
public class DossierControllerV2 {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Dossier created successfully",
                    content = @Content(schema = @Schema(implementation = DossierResponseV2.class))),
            @ApiResponse(responseCode = "400", description = "Invalid request data")
    })
    public ResponseEntity<DossierResponseV2> create(@Valid @RequestBody DossierCreateRequest request) {
    ...
    @GetMapping("/{id:\\d+}")
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(summary = "Get dossier by ID", description = "Retrieves a single dossier by its ID with structured response")
    @ApiResponses(value = {
    ...
    @Operation(
        ...
        description = "Retrieves a paginated list of dossiers with optional filtering and structured response format"
    )
    @ApiResponses(value = {
