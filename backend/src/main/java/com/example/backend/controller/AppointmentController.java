...
public class AppointmentController {
    ...
    @Operation(
            ...
            description = "Creates a new appointment for a dossier")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "201",
    ...
    @Operation(
            ...
            description = "Deletes an appointment by its ID (ADMIN only)")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "204",
    ...
    @Operation(
            ...
            description =
                    "Retrieves a paginated list of appointments with optional filtering by dossier ID, date range, assigned user, and status. Results are sorted by start time in ascending order by default.")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<Page<AppointmentResponse>> list(
            ...
            @Parameter(
                            description =
                                    "Filter by start time from (ISO 8601 format: yyyy-MM-dd'T'HH:mm:ss)")
                    @RequestParam(required = false)
                    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)
                    LocalDateTime from,
            @Parameter(
                            description =
                                    "Filter by end time to (ISO 8601 format: yyyy-MM-dd'T'HH:mm:ss)")
                    @RequestParam(required = false)
                    @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)
                    LocalDateTime to,
            @Parameter(description = "Filter by assigned user") @RequestParam(required = false)
                    String assignedTo,
            @Parameter(description = "Filter by appointment status") @RequestParam(required = false)
