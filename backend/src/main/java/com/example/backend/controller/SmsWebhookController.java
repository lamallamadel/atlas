...
public class SmsWebhookController {
    ...
    public ResponseEntity<String> handleTwilioStatusCallback(
        ...
        try {
            SmsProviderConfig config = providerConfigRepository.findByOrgId(orgId)
                .orElse(null);
            
            if (config == null) {
                logger.warn("No SMS provider config found for orgId={}", orgId);
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Provider config not found");
            }
    ...
    @PostMapping(value = "/{orgId}/aws-sns/delivery", consumes = {MediaType.APPLICATION_JSON_VALUE, "text/plain"})
    public ResponseEntity<Map<String, Object>> handleAwsSnsDeliveryCallback(
        ...
        logger.info("Received AWS SNS delivery webhook for orgId={}, messageType={}", orgId, messageType);
        ...
        try {
            ...
            if ("SubscriptionConfirmation".equals(messageType)) {
                ...
                logger.info("AWS SNS subscription confirmation for orgId={}, URL={}", orgId, subscribeUrl);
                return ResponseEntity.ok(Map.of("status", "confirmed", "subscribeUrl", subscribeUrl));
            }
    ...
    public ResponseEntity<Map<String, Object>> handleGenericDeliveryCallback(
        ...
        try {
            ...
            if (signature != null && config.getWebhookSecretEncrypted() != null) {
                if (!verifyWebhookSignature(payload, signature, config.getWebhookSecretEncrypted())) {
    ...
    private boolean verifyTwilioSignature(Map<String, String> params, String signature, String authToken) {
        try {
            ...
            SecretKeySpec secretKeySpec = new SecretKeySpec(authToken.getBytes(StandardCharsets.UTF_8), "HmacSHA1");
    ...
    private boolean verifyWebhookSignature(Map<String, Object> payload, String signature, String secret) {
        try {
    ...
    private void processTwilioStatusEvent(String orgId, Map<String, String> params) {
        ...
        logger.info("Processing Twilio status event for orgId={}, sid={}, status={}, errorCode={}", 
            orgId, messageSid, messageStatus, errorCode);
        ...
        if ("delivered".equalsIgnoreCase(messageStatus)) {
            ...
            logger.warn("SMS delivery failed: sid={}, status={}, errorCode={}", 
    ...
    private void processAwsSnsNotification(String orgId, Map<String, Object> payload) {
        ...
        try {
            ...
            logger.info("AWS SNS delivery status for orgId={}, messageId={}, status={}", 
    ...
    private void processGenericDeliveryEvent(String orgId, Map<String, Object> payload) {
        ...
        logger.info("Processing generic delivery event for orgId={}, messageId={}, status={}", 
            orgId, messageId, status);
    }
}
