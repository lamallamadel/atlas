...
@Tag(name = "Report Schedules", description = "API for managing scheduled reports with automated email delivery")
public class ReportScheduleController {
    ...
    @PostMapping
    @Operation(summary = "Create report schedule", description = "Create a new scheduled report with daily/weekly/monthly frequency")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Schedule created successfully",
                    content = @Content(schema = @Schema(implementation = ReportScheduleDto.class))),
            @ApiResponse(responseCode = "400", description = "Invalid schedule configuration")
    })
    public ResponseEntity<ReportScheduleDto> createSchedule(@RequestBody ReportScheduleDto schedule) {
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update report schedule", description = "Update an existing scheduled report")
    @ApiResponses(value = {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Get report schedule", description = "Get details of a specific scheduled report")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Enable report schedule", description = "Enable a disabled scheduled report")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Execute report now", description = "Execute a scheduled report immediately")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get execution history", description = "Get execution history for a scheduled report")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Execution history retrieved successfully"),
            @ApiResponse(responseCode = "404", description = "Schedule not found")
    })
    public ResponseEntity<List<Map<String, Object>>> getExecutionHistory(@PathVariable Long scheduleId) {
