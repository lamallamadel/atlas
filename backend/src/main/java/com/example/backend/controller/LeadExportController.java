...
public class LeadExportController {
    ...
    @Operation(summary = "Export leads to CSV", 
               description = "Exports leads to a CSV file with optional filters and configurable columns")
    @ApiResponses(value = {
    ...
    public void exportLeads(
            ...
            @RequestParam(value = "status", required = false) DossierStatus status,
            ...
            @RequestParam(value = "source", required = false) DossierSource source,
            @Parameter(description = "Comma-separated list of columns to include (e.g., id,name,phone,email)")
