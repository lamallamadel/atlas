...
public class WorkflowController {
        ...
        public WorkflowController(
                        WorkflowService workflowService, WorkflowValidationService workflowValidationService) {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Workflow definition created successfully", content = @Content(schema = @Schema(implementation = WorkflowDefinitionResponse.class))),
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Workflow definition found", content = @Content(schema = @Schema(implementation = WorkflowDefinitionResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Workflow definition not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<WorkflowDefinitionResponse> getDefinitionById(
                        @Parameter(description = "ID of the workflow definition", required = true) @PathVariable Long id) {
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "List workflow definitions", description = "Retrieves a paginated list of workflow definitions with optional filtering")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Workflow definitions retrieved successfully", content = @Content(schema = @Schema(implementation = Page.class)))
        })
        public ResponseEntity<Page<WorkflowDefinitionResponse>> listDefinitions(
                        @Parameter(description = "Filter by case type") @RequestParam(required = false) String caseType,
                        @Parameter(description = "Filter by active status") @RequestParam(required = false) Boolean isActive,
                        @Parameter(description = "Page number (0-indexed)") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Page size (min=1, default=20)") @RequestParam(defaultValue = "20") int size,
                ...
                Page<WorkflowDefinitionResponse> response = workflowService.listDefinitions(caseType, isActive,
                                pageable);
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Get transitions for case type", description = "Retrieves all active workflow definitions for a specific case type")
        @ApiResponses(value = {
        ...
        public ResponseEntity<List<WorkflowDefinitionResponse>> getTransitionsForCaseType(
                        @Parameter(description = "Case type code", required = true) @PathVariable String caseType) {
        ...
        public ResponseEntity<WorkflowDefinitionResponse> createNewVersion(
                        @Parameter(description = "ID of the parent workflow", required = true) @PathVariable Long id,
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Workflow definition updated successfully", content = @Content(schema = @Schema(implementation = WorkflowDefinitionResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Workflow definition not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid request data", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "403", description = "Forbidden - Admin role required", content = @Content)
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "204", description = "Workflow definition deleted successfully", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Workflow definition not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "403", description = "Forbidden - Admin role required", content = @Content)
        })
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Get transition history for dossier", description = "Retrieves workflow transition history for a specific dossier")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Transition history retrieved successfully", content = @Content(schema = @Schema(implementation = Page.class)))
        })
        public ResponseEntity<Page<WorkflowTransitionResponse>> getTransitionHistory(
                        ...
                        @Parameter(description = "Page number (0-indexed)") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Page size (min=1, default=20)") @RequestParam(defaultValue = "20") int size,
                        @Parameter(description = "Sort criteria in format: property(,asc|desc)") @RequestParam(defaultValue = "transitionedAt,desc") String sort) {
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Get allowed next statuses", description = "Retrieves allowed next statuses for a given case type and current status")
        @ApiResponses(value = {
        ...
        public ResponseEntity<List<String>> getAllowedNextStatuses(
                        @Parameter(description = "Case type code", required = true) @RequestParam String caseType,
                        @Parameter(description = "Current status", required = true) @RequestParam String currentStatus) {
                List<String> allowedStatuses = workflowValidationService.getAllowedNextStatuses(caseType,
                                currentStatus);
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Validate transition before attempting", description = "Checks if a status transition would be valid without actually performing it. "
        ...
        public ResponseEntity<java.util.Map<String, Object>> validateTransition(
                        @Parameter(description = "Dossier ID", required = true) @PathVariable Long dossierId,
                        @Parameter(description = "Source status", required = true) @RequestParam String fromStatus,
                        @Parameter(description = "Target status", required = true) @RequestParam String toStatus) {
                java.util.Map<String, Object> validationResult = workflowService.validateTransition(dossierId,
                                fromStatus, toStatus);
