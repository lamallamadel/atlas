...
@Tag(
        ...
        description = "Admin API for managing rate limits per organization and IP addresses")
...
public class RateLimitController {
    ...
    @Operation(
            ...
            description = "Retrieve all configured rate limits for organizations")
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(
            summary = "Get rate limit by organization",
            description = "Retrieve rate limit configuration for a specific organization")
    public ResponseEntity<RateLimitTierDto> getRateLimitByOrgId(@PathVariable String orgId) {
        return rateLimitService
                .getRateLimitByOrgId(orgId)
    ...
    @Operation(
            ...
            description = "Create a new rate limit configuration for an organization")
    public ResponseEntity<RateLimitTierDto> createRateLimit(
    ...
    @Operation(
            ...
            description = "Update rate limit configuration for an organization")
    public ResponseEntity<RateLimitTierDto> updateRateLimit(
            @PathVariable String orgId, @Valid @RequestBody RateLimitTierDto dto) {
    ...
    @Operation(
            ...
            description = "Delete rate limit configuration for an organization")
    ...
    @Operation(
            ...
            description =
                    "Clear the rate limit bucket for an organization, allowing immediate new requests")
    public ResponseEntity<Map<String, String>> resetRateLimitBucketForOrg(
        ...
        return ResponseEntity.ok(
                Map.of("message", "Rate limit bucket cleared successfully", "orgId", orgId));
    }
    ...
    @Operation(
            ...
            description =
                    "Clear the rate limit bucket for a specific IP address, allowing immediate new requests")
    public ResponseEntity<Map<String, String>> resetRateLimitBucketForIp(
            @Parameter(description = "IP address to reset (e.g., 192.168.1.1)") @PathVariable
                    String ipAddress) {
        rateLimitService.clearBucketForIp(ipAddress);
        return ResponseEntity.ok(
                Map.of(
                        "message",
                        "Rate limit bucket cleared successfully",
                        "ipAddress",
                        ipAddress));
    }
    ...
    @Operation(
            ...
            description =
                    "Retrieve comprehensive statistics about rate limiting including hits, rejections, and rates")
