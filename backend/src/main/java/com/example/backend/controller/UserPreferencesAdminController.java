...
@Tag(
        name = "User Preferences Admin",
        description = "Admin API for managing user preferences and dashboard customization")
public class UserPreferencesAdminController {

    private final UserPreferencesService userPreferencesService;

    public UserPreferencesAdminController(UserPreferencesService userPreferencesService) {
    ...
    @Operation(
            ...
            description =
                    "Retrieves all preferences for a specific user including dashboard layout and widget settings")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
            ...
            value = {
                @ApiResponse(
                        ...
                        content =
                                @Content(
                                        schema =
                                                @Schema(implementation = UserPreferencesDTO.class)))
            })
    public ResponseEntity<UserPreferencesDTO> updateUserPreferences(
        ...
        UserPreferencesDTO updated =
                userPreferencesService.saveUserPreferences(userId, preferences);
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Apply role-based template",
            description =
                    "Applies a pre-configured dashboard template based on user role (agent, manager, admin)")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    @Operation(
            ...
            description =
                    "Exports the current dashboard configuration as JSON for backup or sharing")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<UserPreferencesDTO> updateTourProgress(
        ...
        UserPreferencesDTO updated =
                userPreferencesService.updateTourProgress(userId, tourProgress);
        return ResponseEntity.ok(updated);
    }
    ...
    public ResponseEntity<Map<String, Object>> getTourProgress(
            @Parameter(description = "User ID", required = true) @PathVariable String userId) {
        UserPreferencesDTO preferences = userPreferencesService.getUserPreferences(userId);
        Map<String, Object> tourProgress =
                preferences.getTourProgress() != null
    ...
    @Operation(
            ...
            description =
                    "Retrieves user preferences for a specific category (ui, notifications, formats, shortcuts) with inheritance from organization and system defaults")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<UserPreferencesDTO.CategoryPreferencesResponse> getPreferencesByCategory(
            @Parameter(description = "User ID", required = true) @PathVariable String userId,
            @Parameter(
                            description = "Category (ui, notifications, formats, shortcuts)",
                            required = true)
                    @PathVariable
                    String category) {
        Map<String, Object> preferences =
                userPreferencesService.getPreferencesByCategory(userId, category);
        return ResponseEntity.ok(
                new UserPreferencesDTO.CategoryPreferencesResponse(category, preferences));
    }

    @PutMapping("/{userId}/category/{category}")
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Set preferences by category",
            description =
                    "Sets user preferences for a specific category with JSON schema validation")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                ...
                @ApiResponse(
                        responseCode = "400",
                        description = "Invalid category or preferences schema validation failed")
            })
    public ResponseEntity<UserPreferencesDTO.CategoryPreferencesResponse> setPreferencesByCategory(
        ...
        Map<String, Object> updated =
                userPreferencesService.setPreferencesByCategory(
                        userId, category, request.getPreferences());
        return ResponseEntity.ok(
                new UserPreferencesDTO.CategoryPreferencesResponse(category, updated));
    }

    @DeleteMapping("/{userId}/category/{category}")
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Reset preferences by category",
            description =
                    "Resets user preferences for a specific category to organization/system defaults")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                        description = "Category preferences reset successfully"),
                @ApiResponse(responseCode = "400", description = "Invalid category")
            })
    public ResponseEntity<UserPreferencesDTO.CategoryPreferencesResponse>
            resetPreferencesByCategory(
                    @Parameter(description = "User ID", required = true) @PathVariable
                            String userId,
                    @Parameter(
                                    description =
                                            "Category (ui, notifications, formats, shortcuts)",
                                    required = true)
                            @PathVariable
                            String category) {
        Map<String, Object> defaults =
                userPreferencesService.resetPreferencesByCategory(userId, category);
        return ResponseEntity.ok(
                new UserPreferencesDTO.CategoryPreferencesResponse(category, defaults));
    }

    @GetMapping("/{userId}/all-categories")
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Get all preferences by categories",
            description =
                    "Retrieves all user preferences organized by categories with inheritance applied")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
