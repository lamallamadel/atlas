...
@Deprecated(
        ...
        deprecationMessage =
                "API v1 is deprecated. Please migrate to /api/v2/dossiers which provides structured responses with nested objects.")
public class DossierController {
    ...
    @ApiResponses(
            value = {
                @ApiResponse(
                        ...
                        content =
                                @Content(schema = @Schema(implementation = DossierResponse.class))),
                @ApiResponse(
                        responseCode = "400",
    ...
    public ResponseEntity<DossierResponse> create(
        ...
        if (request.getInitialParty() != null && request.getInitialParty().getPhone() != null) {
            var duplicates =
                    dossierService.checkForDuplicates(request.getInitialParty().getPhone());
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Match Off-Market",
            description = "Target off-market properties for this dossier")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    @Operation(
            ...
            description =
                    "Retrieves a paginated list of dossiers with optional filtering. "
    ...
    public ResponseEntity<Page<DossierResponse>> list(
        ...
        Page<DossierResponse> response =
                dossierService.list(status, leadPhone, annonceId, pageable);
        return ResponseEntity.ok(response);
    }
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Update dossier lead information",
            description = "Updates the lead name and phone of an existing dossier")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                        description = "Dossier lead information updated successfully",
                        content =
                                @Content(schema = @Schema(implementation = DossierResponse.class))),
                @ApiResponse(
                        responseCode = "404",
    ...
    @Operation(
            ...
            description =
                    "Retrieves the status transition history for a specific dossier. "
    ...
    public ResponseEntity<Page<DossierStatusHistoryResponse>> getStatusHistory(
            ...
            @Parameter(
                            description =
                                    "Sort criteria in format: property(,asc|desc). Default sort order is descending by transitionedAt.")
                    @RequestParam(defaultValue = "transitionedAt,desc")
                    String sort) {
        Pageable pageable = createPageable(page, size, sort);
        Page<DossierStatusHistoryResponse> history =
                statusHistoryService.getStatusHistory(id, pageable);
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Bulk assign dossier status",
            description = "Updates the status of multiple dossiers with validation")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Advanced filter dossiers",
            description =
                    "Filters dossiers using advanced query builder with AND/OR logic and multiple operators")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
                        description = "Dossiers filtered successfully",
                        content = @Content(schema = @Schema(implementation = Page.class)))
            })
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
    @Operation(
            summary = "Count dossiers with advanced filters",
            description =
                    "Returns the count of dossiers matching advanced filter criteria in real-time")
    @ApiResponses(
            value = {
