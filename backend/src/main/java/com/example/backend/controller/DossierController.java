...
public class DossierController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Dossier created successfully", content = @Content(schema = @Schema(implementation = DossierResponse.class))),
                        ...
                        @ApiResponse(responseCode = "404", description = "Related resource not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<DossierResponse> create(
                ...
                // Let GlobalExceptionHandler standardize all error responses as RFC 7807
                // ProblemDetail.
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Dossier found", content = @Content(schema = @Schema(implementation = DossierResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Dossier not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<DossierResponse> getById(
                        @Parameter(description = "ID of the dossier to retrieve", required = true) @PathVariable Long id) {
                DossierResponse response = dossierService.getById(id);
                return ResponseEntity.ok(response);
        }

        @GetMapping("/{id:\\d+}/match-off-market")
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Match Off-Market", description = "Target off-market properties for this dossier")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Matches found", content = @Content(schema = @Schema(implementation = com.example.backend.brain.dto.MatchResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Dossier not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<com.example.backend.brain.dto.MatchResponse> matchOffMarket(
                        @Parameter(description = "ID of the dossier", required = true) @PathVariable Long id) {
                com.example.backend.brain.dto.MatchResponse response = dossierService.matchOffMarket(id);
                if (response == null) {
        ...
        public ResponseEntity<List<DossierResponse>> checkDuplicates(
                        @RequestParam("leadPhone") String leadPhone) {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Dossiers retrieved successfully", content = @Content(schema = @Schema(implementation = Page.class)))
        })
        public ResponseEntity<Page<DossierResponse>> list(
                        @Parameter(description = "Filter by dossier status") @RequestParam(required = false) DossierStatus status,
                        @Parameter(description = "Filter by lead phone number") @RequestParam(required = false) String leadPhone,
                        @Parameter(description = "Filter by annonce ID") @RequestParam(required = false) Long annonceId,
                        @Parameter(description = "Page number (0-indexed)") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Page size (min=1, default=20)", example = "20") @RequestParam(defaultValue = "20") int size,
                        @Parameter(description = "Sort criteria in format: property(,asc|desc). Default sort order is ascending. Multiple sort criteria are supported.") @RequestParam(defaultValue = "id,asc") String sort) {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Dossier status updated successfully", content = @Content(schema = @Schema(implementation = DossierResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Dossier not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid request data", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<DossierResponse> patchStatus(
                        @Parameter(description = "ID of the dossier to update", required = true) @PathVariable Long id,
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Dossier lead information updated successfully", content = @Content(schema = @Schema(implementation = DossierResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Dossier not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid request data", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Status history retrieved successfully", content = @Content(schema = @Schema(implementation = Page.class))),
                        @ApiResponse(responseCode = "404", description = "Dossier not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        public ResponseEntity<Page<DossierStatusHistoryResponse>> getStatusHistory(
                        ...
                        @Parameter(description = "Page number (0-indexed)") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Page size (min=1, default=20)", example = "20") @RequestParam(defaultValue = "20") int size,
                        @Parameter(description = "Sort criteria in format: property(,asc|desc). Default sort order is descending by transitionedAt.") @RequestParam(defaultValue = "transitionedAt,desc") String sort) {
                Pageable pageable = createPageable(page, size, sort);
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Bulk assign completed", content = @Content(schema = @Schema(implementation = BulkOperationResponse.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid request data", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "204", description = "Dossier deleted successfully", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Dossier not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                        @ApiResponse(responseCode = "403", description = "Forbidden - Admin role required", content = @Content)
        })
        public ResponseEntity<Void> delete(
                        @Parameter(description = "ID of the dossier to delete", required = true) @PathVariable Long id) {
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Get allowed status codes for a case type", description = "Retrieves the list of valid status codes for a specific case type based on workflow definitions")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Status codes retrieved successfully", content = @Content(schema = @Schema(implementation = List.class)))
        })
        public ResponseEntity<List<String>> getAllowedStatusCodes(
                        @Parameter(description = "Case type code (optional, returns all active status codes if not specified)") @RequestParam(required = false) String caseType) {
                List<String> allowedStatusCodes = statusCodeValidationService
                                .getAllowedStatusCodesForCaseType(caseType);
                return ResponseEntity.ok(allowedStatusCodes);
        }

        @PostMapping("/advanced-filter")
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Advanced filter dossiers", description = "Filters dossiers using advanced query builder with AND/OR logic and multiple operators")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Dossiers filtered successfully", content = @Content(schema = @Schema(implementation = Page.class)))
        })
        public ResponseEntity<Page<DossierResponse>> advancedFilter(
                        @Valid @RequestBody DossierFilterRequest filterRequest) {
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Count dossiers with advanced filters", description = "Returns the count of dossiers matching advanced filter criteria in real-time")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Count retrieved successfully")
        })
        public ResponseEntity<Long> countAdvancedFilter(
                        @Valid @RequestBody DossierFilterRequest filterRequest) {
