...
public class DeveloperPortalController {
    ...
    public ResponseEntity<List<ApiKeyResponse>> listApiKeys(
        ...
        List<ApiKeyResponse> keys =
                apiKeyService.getApiKeysByOrg(orgId).stream()
    ...
    public ResponseEntity<List<Map<String, Object>>> getApiKeyUsage(
        ...
        List<Map<String, Object>> usage =
                usageTrackingService.getUsageByApiKey(id, startDate, endDate).stream()
                        .map(
                                u ->
    ...
    public ResponseEntity<List<WebhookSubscriptionResponse>> listWebhookSubscriptions(
        ...
        List<WebhookSubscriptionResponse> subscriptions =
                webhookService.getSubscriptionsByOrg(orgId).stream()
    ...
    public ResponseEntity<Page<Map<String, Object>>> getWebhookDeliveries(
            @RequestAttribute("orgId") String orgId, @PathVariable Long id, Pageable pageable) {
        Page<Map<String, Object>> deliveries =
                webhookService
                        .getDeliveries(id, orgId, pageable)
                        .map(
                                d ->
                                        Map.<String, Object>of(
                                                "id", d.getId(),
                                                ...
                                                "responseStatusCode",
                                                        d.getResponseStatusCode() != null
                                                                ? d.getResponseStatusCode()
                                                                : 0,
                                                "errorMessage",
                                                        d.getErrorMessage() != null
                                                                ? d.getErrorMessage()
                                                                : ""));
