...
public class ReferentialController {
    ...
    @GetMapping
    @Operation(summary = "Get all referentials by category", description = "Returns all referential items for a given category")
    public ResponseEntity<List<ReferentialResponse>> getAllByCategory(
            @Parameter(description = "Referential category (e.g., CASE_TYPE, CASE_STATUS, LEAD_SOURCE, LOSS_REASON)")
        ...
        List<ReferentialResponse> responses = entities.stream()
                .map(ReferentialMapper::toResponse)
    ...
    @GetMapping("/by-code")
    @Operation(summary = "Get referential by category and code", description = "Returns a referential item by its category and code")
    ...
    @Operation(summary = "Update referential", description = "Updates an existing referential item (system items cannot be modified)")
    public ResponseEntity<ReferentialResponse> update(
            @Parameter(description = "Referential ID") @PathVariable Long id,
            @Parameter(description = "Updated referential data") @Valid @RequestBody ReferentialRequest request) {
