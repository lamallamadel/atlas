...
public class SystemConfigController {
    ...
    @Operation(summary = "Get all system configuration parameters", 
               description = "Retrieves all system configuration parameters with their values (Super Admin only)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Configuration parameters retrieved successfully",
                    content = @Content(schema = @Schema(implementation = SystemConfigResponse.ConfigListResponse.class))),
    ...
    public ResponseEntity<SystemConfigResponse.ConfigListResponse> getAllSystemConfigs() {
        ...
        SystemConfigResponse.ConfigListResponse response = systemConfigService.getAllSystemConfigs();
        logger.info("Retrieved {} system configuration parameters", response.getTotalCount());
        return ResponseEntity.ok(response);
    }

    @PutMapping("/{key}")
    @Operation(summary = "Update specific system configuration parameter", 
               description = "Updates a specific system configuration parameter by key with validation (Super Admin only)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Configuration parameter updated successfully",
    ...
    public ResponseEntity<SystemConfigResponse> updateSystemConfigByKey(
        ...
        SystemConfigResponse response = systemConfigService.updateSystemConfigByKey(key, request.getValue());
        
        logger.info("System configuration parameter '{}' updated successfully by user: {}", key, userId);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/reload")
    @Operation(summary = "Reload configuration without restart", 
               description = "Reloads system configuration from database without requiring application restart (Super Admin only)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Configuration reloaded successfully",
                    content = @Content(schema = @Schema(implementation = SystemConfigResponse.ConfigReloadResponse.class))),
            @ApiResponse(responseCode = "403", description = "Access denied - Super Admin role required"),
            @ApiResponse(responseCode = "401", description = "User is not authenticated")
    })
    public ResponseEntity<SystemConfigResponse.ConfigReloadResponse> reloadConfiguration() {
        String userId = extractUserId();
        logger.info("POST /api/v1/system/config/reload - User {} requesting configuration reload", userId);
        
        SystemConfigResponse.ConfigReloadResponse response = systemConfigService.reloadConfiguration();
        
        logger.info("System configuration reloaded successfully by user {}: {} parameters", 
            userId, response.getConfigsReloaded());
        return ResponseEntity.ok(response);
    }

    @GetMapping("/health")
    @Operation(summary = "Check configuration health", 
               description = "Verifies the validity and health of current system configuration (Super Admin only)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Configuration health check completed",
                    content = @Content(schema = @Schema(implementation = SystemConfigResponse.ConfigHealthResponse.class))),
            @ApiResponse(responseCode = "403", description = "Access denied - Super Admin role required"),
            @ApiResponse(responseCode = "401", description = "User is not authenticated")
    })
    public ResponseEntity<SystemConfigResponse.ConfigHealthResponse> checkConfigHealth() {
        String userId = extractUserId();
        logger.debug("GET /api/v1/system/config/health - User {} checking configuration health", userId);
        
        SystemConfigResponse.ConfigHealthResponse response = systemConfigService.checkConfigHealth();
        
        logger.info("Configuration health check completed by user {}: status={}, valid={}, errors={}", 
            userId, response.getStatus(), response.isValid(), response.getErrors().size());
        return ResponseEntity.ok(response);
    }

    @GetMapping("/legacy")
    @Operation(summary = "Get legacy system configuration", 
               description = "Retrieves system configuration in legacy format for backward compatibility (Super Admin only)")
    @ApiResponses(value = {
    ...
    @PutMapping("/legacy")
    @Operation(summary = "Update legacy system configuration", 
               description = "Updates system configuration in legacy format for backward compatibility (Super Admin only)")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Void> resetToDefaults() {
        logger.info("POST /api/v1/system/config/reset - Resetting system configuration to defaults");
