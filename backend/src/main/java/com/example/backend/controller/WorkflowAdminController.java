...
@Tag(name = "Workflow Admin - Referentials", description = "Admin operations for managing referential data with versioning and templates")
public class WorkflowAdminController {
    ...
    public WorkflowAdminController(ReferentialService referentialService,
    ...
    @GetMapping
    @Operation(summary = "Get all referentials by category", description = "Returns all referential items for a given category with admin view")
    public ResponseEntity<List<ReferentialResponse>> getAllByCategory(
            ...
            @Parameter(description = "Filter only active items") @RequestParam(required = false, defaultValue = "false") boolean activeOnly) {
        
        List<ReferentialEntity> entities = activeOnly
                ? referentialService.getActiveByCategory(category)
                : referentialService.getAllByCategory(category);
        
        List<ReferentialResponse> responses = entities.stream()
                .map(this::toResponse)
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Get referential by ID", description = "Returns a single referential item by its ID")
    ...
    public ResponseEntity<ReferentialResponse> create(
            @Parameter(description = "Referential data") @Valid @RequestBody ReferentialRequest request) {
    ...
    @Operation(summary = "Update referential with versioning", description = "Updates an existing referential item with change tracking")
    public ResponseEntity<ReferentialResponse> update(
            @Parameter(description = "Referential ID") @PathVariable Long id,
            @Parameter(description = "Updated referential data") @Valid @RequestBody ReferentialUpdateRequest request) {
        ...
        ReferentialEntity updated = referentialService.update(id, entity, request.getChangeReason());
    ...
    @Operation(summary = "Delete referential", description = "Deletes a referential item (system items cannot be deleted)")
    ...
    @Operation(summary = "Get version history", description = "Returns the version history for a referential item")
    public ResponseEntity<List<ReferentialVersionResponse>> getVersionHistory(
        ...
        List<ReferentialVersionResponse> responses = versions.stream()
    ...
    @Operation(summary = "Get category version history", description = "Returns all version history for a category")
    public ResponseEntity<List<ReferentialVersionResponse>> getCategoryVersionHistory(
        ...
        List<ReferentialVersionEntity> versions = referentialService.getCategoryVersionHistory(category);
        List<ReferentialVersionResponse> responses = versions.stream()
                .map(this::toVersionResponse)
                .collect(Collectors.toList());
    ...
    @Operation(summary = "Seed default referentials", description = "Seeds default referential values for the current organization")
    ...
    @GetMapping("/export")
    @Operation(summary = "Export referentials as template", description = "Exports referential values as a JSON template")
    public ResponseEntity<ReferentialTemplateDto> exportTemplate(
        ...
        ReferentialTemplateDto template = categories != null && !categories.isEmpty()
    ...
    @Operation(summary = "Import referentials from template", description = "Imports referential values from a JSON template")
    public ResponseEntity<Map<String, Object>> importTemplate(
            @Parameter(description = "Template data") @Valid @RequestBody ReferentialTemplateDto template,
    ...
    @Operation(summary = "Get available categories", description = "Returns list of available referential categories")
