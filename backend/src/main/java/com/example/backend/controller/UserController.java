...
public class UserController {
    ...
    @Operation(summary = "Get user display name", 
               description = "Retrieves the display name for a given user ID. Returns 'Système' for null/system users, " +
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Display name retrieved successfully",
                    content = @Content(schema = @Schema(implementation = UserDisplayNameResponse.class)))
    })
    public ResponseEntity<UserDisplayNameResponse> getDisplayName(
            @Parameter(description = "ID of the user to resolve", required = true)
