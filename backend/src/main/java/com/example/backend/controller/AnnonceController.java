...
public class AnnonceController {
        ...
        @Nullable
        private final com.example.backend.service.YieldManagementService yieldManagementService;
        ...
        public AnnonceController(
                        ...
                        @Autowired(required = false) @Nullable com.example.backend.service.YieldManagementService yieldManagementService) {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Annonce created successfully", content = @Content(schema = @Schema(implementation = AnnonceResponse.class))),
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Annonce found", content = @Content(schema = @Schema(implementation = AnnonceResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Annonce not found", content = @Content)
        })
        public ResponseEntity<AnnonceResponse> getById(
                        @Parameter(description = "ID of the annonce to retrieve", required = true) @PathVariable Long id) {
                try {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Annonce updated successfully", content = @Content(schema = @Schema(implementation = AnnonceResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Annonce not found", content = @Content),
                        @ApiResponse(responseCode = "400", description = "Invalid request data", content = @Content)
        })
        public ResponseEntity<AnnonceResponse> update(
                        @Parameter(description = "ID of the annonce to update", required = true) @PathVariable Long id,
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Annonces retrieved successfully", content = @Content(schema = @Schema(implementation = Page.class)))
        })
        public ResponseEntity<Page<AnnonceResponse>> list(
                        @Parameter(description = "Filter by annonce status") @RequestParam(required = false) AnnonceStatus status,
                        @Parameter(description = "Search query to filter annonces by title, description, category, or city") @RequestParam(required = false) String q,
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "204", description = "Annonce deleted successfully", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Annonce not found", content = @Content),
                        @ApiResponse(responseCode = "403", description = "Forbidden - Admin role required", content = @Content)
        })
        public ResponseEntity<Void> delete(
                        @Parameter(description = "ID of the annonce to delete", required = true) @PathVariable Long id) {
                try {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Bulk update completed", content = @Content(schema = @Schema(implementation = BulkOperationResponse.class))),
                        @ApiResponse(responseCode = "400", description = "Invalid request data", content = @Content)
        })
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Annonces retrieved successfully", content = @Content(schema = @Schema(implementation = CursorPageResponse.class)))
        })
        public ResponseEntity<CursorPageResponse<Annonce>> listWithCursor(
                        @Parameter(description = "Cursor from previous page") @RequestParam(required = false) String cursor,
                ...
                CursorPageResponse<Annonce> response = cursorPaginationService.findWithCursor(Annonce.class,
                                pageRequest, null);
                return ResponseEntity.ok(response);
        }
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Trigger AI scoring", description = "Triggers asynchronous AI quality scoring for an annonce")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "202", description = "Scoring triggered (asynchronous)", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Annonce not found", content = @Content)
        })
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Detect duplicates", description = "Detects duplicate annonces using semantic similarity (TF-IDF + cosine)")
        @ApiResponses(value = {
        ...
        public ResponseEntity<List<DoublonResultDto>> detectDuplicates(
                ...
                List<DoublonResultDto> results = brainClientService != null ? brainClientService.detectDuplicates(dtos)
                                : List.of();
        ...
        @PreAuthorize("hasAnyRole('ADMIN', 'PRO')")
        @Operation(summary = "Trigger fraud analysis", description = "Triggers asynchronous fraud detection for an annonce")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "202", description = "Fraud analysis triggered (asynchronous)", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Annonce not found", content = @Content)
        })
        ...
        @PostMapping("/yield/run")
        @PreAuthorize("hasRole('ADMIN')")
        @Operation(summary = "Trigger Yield Management", description = "Manually triggers the yield management evaluation for all active annonces")
        @ApiResponses(value = { @ApiResponse(responseCode = "202", description = "Yield evaluation started") })
        public ResponseEntity<Void> triggerYieldManagement() {
                if (yieldManagementService != null) {
                        new Thread(yieldManagementService::evaluateYield).start();
                }
                return ResponseEntity.accepted().build();
        }
