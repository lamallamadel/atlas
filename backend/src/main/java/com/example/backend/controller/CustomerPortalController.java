...
public class CustomerPortalController {
    ...
    @Autowired private MagicLinkAuthService magicLinkAuthService;

    @Autowired private ClientSecureMessageRepository messageRepository;

    @Autowired private ClientAppointmentRequestRepository appointmentRequestRepository;

    @Autowired private ClientSatisfactionSurveyRepository surveyRepository;

    @Autowired private ConsentementRepository consentementRepository;

    @Autowired private WhiteLabelConfigRepository whiteLabelConfigRepository;
    ...
    public ResponseEntity<CustomerPortalDossierDTO> getDossier(
            @PathVariable Long dossierId, @RequestHeader("Authorization") String authHeader) {
        ...
        CustomerPortalDossierDTO dossier =
                customerPortalService.getDossierForClient(dossierId, authToken.getOrgId());
    ...
    public ResponseEntity<List<ClientSecureMessage>> getMessages(
        ...
        List<ClientSecureMessage> messages =
                messageRepository.findByOrgIdAndDossierIdOrderByCreatedAtAsc(
                        authToken.getOrgId(), dossierId);
    ...
    public ResponseEntity<Void> markMessageAsRead(
            @PathVariable Long messageId, @RequestHeader("Authorization") String authHeader) {

        String token = extractToken(authHeader);
        ClientAuthToken authToken = magicLinkAuthService.validateToken(token);

        ClientSecureMessage message =
                messageRepository
                        .findById(messageId)
    ...
    public ResponseEntity<List<ClientAppointmentRequest>> getAppointmentRequests(
        ...
        List<ClientAppointmentRequest> requests =
                appointmentRequestRepository.findByOrgIdAndDossierIdOrderByCreatedAtDesc(
                        authToken.getOrgId(), dossierId);
    ...
    public ResponseEntity<List<ClientSatisfactionSurvey>> getSurveys(
        ...
        List<ClientSatisfactionSurvey> surveys =
                surveyRepository.findByOrgIdAndDossierIdOrderByCreatedAtDesc(
                        authToken.getOrgId(), dossierId);
    ...
    public ResponseEntity<WhiteLabelConfigEntity> getBranding(@PathVariable String orgId) {
        WhiteLabelConfigEntity config = whiteLabelConfigRepository.findByOrgId(orgId).orElse(null);
    ...
    public ResponseEntity<Resource> downloadDocument(
            @PathVariable Long documentId, @RequestHeader("Authorization") String authHeader) {
    ...
    public ResponseEntity<List<Annonce>> getPropertyRecommendations(
        ...
        List<Annonce> recommendations =
                annonceRepository.findTop10ByOrgIdOrderByCreatedAtDesc(authToken.getOrgId());
