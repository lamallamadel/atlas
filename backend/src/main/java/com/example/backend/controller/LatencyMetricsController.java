...
public class LatencyMetricsController {
    ...
    public ResponseEntity<Map<String, String>> recordLatency(@RequestBody LatencyMetric metric) {
        try {
            ...
            Timer timer = latencyTimers.computeIfAbsent(
                metric.getRegion(),
                region -> Timer.builder(timerName)
                    .tag("region", region)
            ...
            if (metric.getLatency() > 200) {
                logger.warn("High latency detected: {}ms for region {} ({})", 
                    metric.getLatency(), metric.getRegion(), metric.getMetric());
                
                meterRegistry.counter("high.latency.events",
