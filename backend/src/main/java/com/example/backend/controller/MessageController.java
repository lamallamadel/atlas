...
public class MessageController {
    ...
    @Operation(
            ...
            description = "Creates a new message for a dossier")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "201",
    ...
    public ResponseEntity<MessageResponse> getById(
            @Parameter(description = "ID of the message to retrieve", required = true) @PathVariable
                    Long id) {
        try {
    ...
    @Operation(
            ...
            description =
                    "Retrieves a paginated list of messages with optional filtering by dossier, channel, direction, and date range. Results are sorted by timestamp in descending order by default.")
    @ApiResponses(
            value = {
                @ApiResponse(
                        responseCode = "200",
    ...
    public ResponseEntity<Page<MessageResponse>> list(
            @Parameter(description = "Filter by dossier ID", required = true) @RequestParam
                    Long dossierId,
            @Parameter(description = "Filter by message channel") @RequestParam(required = false)
                    MessageChannel channel,
            @Parameter(description = "Filter by message direction") @RequestParam(required = false)
            ...
            @Parameter(
                            description =
                                    "Sort criteria in format: property(,asc|desc). Default sort order is descending. Multiple sort criteria are supported.")
                    @RequestParam(defaultValue = "timestamp,desc")
                    String sort) {

        Pageable pageable = createPageable(page, size, sort);
        Page<MessageResponse> response =
                messageService.listByDossier(
