...
export interface IllustrationLibraryItem {
  name: string;
  context: string;
  description: string;
  svg: string;
...
/**
 * Illustration Library Service
 * Manages 15+ custom SVG illustrations for empty states
...
export class IllustrationLibraryService {
  private illustrations: Map<string, IllustrationLibraryItem> = new Map();
  ...
  private initializeLibrary(): void {
    const items: IllustrationLibraryItem[] = [
      {
        name: 'dossier',
        context: 'NO_DOSSIERS',
        description: 'Folder with documents',
        svg: this.getDossierSvg()
      },
      {
        name: 'annonce',
        context: 'NO_ANNONCES',
        description: 'House illustration',
        svg: this.getAnnonceSvg()
      },
      {
        name: 'message',
        context: 'NO_MESSAGES',
        description: 'Chat bubbles',
        svg: this.getMessageSvg()
      },
      {
        name: 'search',
        context: 'NO_SEARCH_RESULTS',
        description: 'Magnifying glass',
        svg: this.getSearchSvg()
      },
      {
        name: 'calendar',
        context: 'NO_APPOINTMENTS',
        description: 'Calendar with clock',
        svg: this.getCalendarSvg()
      },
      {
        name: 'task',
        context: 'NO_TASKS',
        description: 'Clipboard with checklist',
        svg: this.getTaskSvg()
      },
      {
        name: 'document',
        context: 'NO_DOCUMENTS',
        description: 'Document stack',
        svg: this.getDocumentSvg()
      },
      {
        name: 'activity',
        context: 'NO_ACTIVITIES',
        description: 'Activity pulse',
        svg: this.getActivitySvg()
      },
      {
        name: 'notification',
        context: 'NO_NOTIFICATIONS',
        description: 'Bell with checkmark',
        svg: this.getNotificationSvg()
      },
      {
        name: 'network-error',
        context: 'NETWORK_ERROR',
        description: 'Cloud with error',
        svg: this.getNetworkErrorSvg()
      },
      {
        name: 'import-success',
        context: 'IMPORT_SUCCESS',
        description: 'Folder with arrow down',
        svg: this.getImportSuccessSvg()
      },
      {
        name: 'export-success',
        context: 'EXPORT_SUCCESS',
        description: 'Document with arrow up',
        svg: this.getExportSuccessSvg()
      },
      {
        name: 'maintenance',
        context: 'MAINTENANCE',
        description: 'Tools and cones',
        svg: this.getMaintenanceSvg()
      },
      {
        name: 'no-data',
        context: 'NO_DATA',
        description: 'Empty database',
        svg: this.getNoDataSvg()
      },
      {
        name: 'empty-search',
        context: 'EMPTY_SEARCH',
        description: 'Search bar with cursor',
        svg: this.getEmptySearchSvg()
      },
      {
        name: 'favorites',
        context: 'NO_FAVORITES',
        description: 'Heart with sparkles',
        svg: this.getFavoritesSvg()
      },
      {
        name: 'calendar-events',
        context: 'NO_CALENDAR_EVENTS',
        description: 'Empty calendar',
        svg: this.getCalendarEventsSvg()
      }
    ];

    items.forEach(item => {
      this.illustrations.set(item.name, item);
    });
  }

  getIllustration(name: string): SafeHtml | null {
    const item = this.illustrations.get(name);
    return item ? this.sanitizer.bypassSecurityTrustHtml(item.svg) : null;
  }

  getIllustrationByContext(context: string): SafeHtml | null {
    const item = Array.from(this.illustrations.values()).find(i => i.context === context);
    return item ? this.sanitizer.bypassSecurityTrustHtml(item.svg) : null;
  }
  ...
  private getDossierSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        ...
        <path d="M50 70 L50 150 L150 150 L150 80 L120 80 L110 70 Z" fill="url(#folder-grad)" class="folder-main"/>
        <path d="M50 70 L110 70 L115 75 L150 75 L150 80 L120 80 L110 70 Z" fill="url(#folder-grad)" opacity="0.8"/>
        <rect x="70" y="90" width="60" height="45" rx="3" fill="url(#doc-grad)" class="document-pulse"/>
        ...
        <style>
          ...
          @keyframes scaleIn { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } }
          .pulse-circle { animation: pulse 3s ease-in-out infinite; }
          .document-pulse { animation: documentFloat 2s ease-in-out infinite; }
          .plus-icon { animation: scaleIn 2s ease-in-out infinite; transform-origin: 140px 60px; }
          .folder-main { filter: drop-shadow(0 4px 6px rgba(0, 0, 0, 0.1)); }
        </style>
      </svg>
    `;
  }

  private getAnnonceSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <style>
          @keyframes houseFloat { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-8px); } }
          @keyframes windowBlink { 0%, 90%, 100% { opacity: 0.9; } 95% { opacity: 0.5; } }
          .house-float { animation: houseFloat 3s ease-in-out infinite; }
          .window-blink { animation: windowBlink 4s ease-in-out infinite; }
          .plus-icon { animation: scaleIn 2s ease-in-out infinite; transform-origin: 150px 70px; }
        </style>
      </svg>
    `;
  }

  private getMessageSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <g class="send-icon">
          ...
          <path d="M142 60 L158 60 L150 52 M158 60 L150 68" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
        </g>
        <style>
          @keyframes floatBubble1 { 0%, 100% { transform: translate(0, 0); } 50% { transform: translate(-3px, -5px); } }
          @keyframes floatBubble2 { 0%, 100% { transform: translate(0, 0); } 50% { transform: translate(3px, 5px); } }
          @keyframes typingDot { 0%, 60%, 100% { transform: translateY(0); opacity: 0.7; } 30% { transform: translateY(-5px); opacity: 1; } }
          .bubble-float-1 { animation: floatBubble1 3s ease-in-out infinite; }
          .bubble-float-2 { animation: floatBubble2 3s ease-in-out infinite; }
          ...
          .send-icon { animation: scaleIn 2s ease-in-out infinite; transform-origin: 150px 60px; }
        </style>
      </svg>
    `;
  }

  private getSearchSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <g class="question-mark">
          <path d="M85 75 Q85 65 95 65 Q105 65 105 75 Q105 85 95 90" stroke="#667eea" stroke-width="3" fill="none" stroke-linecap="round"/>
        ...
        <style>
          @keyframes searchRotate { 0%, 100% { transform: rotate(0deg); } 25% { transform: rotate(-5deg); } 75% { transform: rotate(5deg); } }
          @keyframes particleFloat { 0%, 100% { transform: translateY(0); opacity: 0.6; } 50% { transform: translateY(-10px); opacity: 1; } }
          .search-glass { animation: searchRotate 3s ease-in-out infinite; transform-origin: 100px 100px; }
          .question-mark { animation: scaleIn 2s ease-in-out infinite; transform-origin: 95px 85px; }
  ...
  private getCalendarSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        ...
        <style>
          @keyframes calendarFloat { 0%, 100% { transform: translateY(0) rotate(0deg); } 50% { transform: translateY(-5px) rotate(-1deg); } }
          @keyframes datePulse { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.2); opacity: 0.8; } }
          .calendar-float { animation: calendarFloat 3s ease-in-out infinite; }
          .date-pulse { animation: datePulse 2s ease-in-out infinite; }
          .clock-icon { animation: scaleIn 2s ease-in-out infinite; transform-origin: 145px 60px; }
        </style>
      </svg>
    `;
  }
  ...
  private getTaskSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        ...
        <style>
          @keyframes clipboardFloat { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-6px); } }
          @keyframes taskAppear { 0% { opacity: 0; transform: translateX(-10px); } 100% { opacity: 1; transform: translateX(0); } }
          .clipboard-float { animation: clipboardFloat 3s ease-in-out infinite; }
          ...
          .plus-icon { animation: scaleIn 2s ease-in-out infinite; transform-origin: 145px 140px; }
        </style>
      </svg>
    `;
  }

  private getDocumentSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <g class="upload-icon">
          ...
          <path d="M145 52 L145 68 M145 52 L140 57 M145 52 L150 57" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
        </g>
        <style>
          @keyframes docFloat { 0%, 100% { transform: translateY(0) rotate(0deg); } 50% { transform: translateY(-8px) rotate(2deg); } }
          .doc-main { animation: docFloat 3s ease-in-out infinite; filter: drop-shadow(0 4px 8px rgba(0, 0, 0, 0.1)); }
          .upload-icon { animation: scaleIn 2s ease-in-out infinite; transform-origin: 145px 60px; }
        </style>
      </svg>
    `;
  }

  private getActivitySvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        ...
        <style>
          @keyframes pulseRing { 0% { r: 25; opacity: 1; } 100% { r: 70; opacity: 0; } }
  ...
  private getNotificationSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <circle cx="100" cy="100" r="80" fill="#f5f3ff" class="pulse-circle"/>
        <g class="bell-swing">
          <path d="M100 70 Q85 75 85 95 L85 110 Q85 115 80 120 L120 120 Q115 115 115 110 L115 95 Q115 75 100 70" fill="url(#bell-grad)"/>
        ...
        <style>
          @keyframes bellSwing { 0%, 100% { transform: rotate(0deg); } 25% { transform: rotate(-10deg); } 75% { transform: rotate(10deg); } }
          .bell-swing { animation: bellSwing 2s ease-in-out infinite; transform-origin: 100px 70px; }
        </style>
      </svg>
    `;
  }

  private getNetworkErrorSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <style>
          @keyframes cloudFloat { 0%, 100% { transform: translateX(0); } 50% { transform: translateX(-5px); } }
          @keyframes scaleIn { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } }
          .cloud-float { animation: cloudFloat 3s ease-in-out infinite; }
          .error-cross { animation: scaleIn 2s ease-in-out infinite; transform-origin: 135px 70px; }
        </style>
      </svg>
    `;
  }

  private getImportSuccessSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <circle cx="100" cy="100" r="80" fill="#f0fdf4" class="pulse-circle"/>
        <g class="folder-anim">
          <path d="M50 90 L50 150 L150 150 L150 100 L120 100 L110 90 Z" fill="url(#success-grad)" opacity="0.3"/>
          <path d="M50 90 L110 90 L115 95 L150 95 L150 100 L120 100 L110 90 Z" fill="url(#success-grad)" opacity="0.5"/>
        </g>
        <g class="arrow-down">
        ...
        <style>
          ...
          @keyframes sparkle { 0%, 100% { opacity: 0.8; transform: scale(1); } 50% { opacity: 1; transform: scale(1.5); } }
          .arrow-down { animation: arrowBounce 1.5s ease-in-out infinite; }
          ...
          .check-badge { animation: scaleIn 2s ease-in-out infinite; transform-origin: 135px 135px; }
        </style>
      </svg>
    `;
  }

  private getExportSuccessSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <style>
          @keyframes arrowRise { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-5px); } }
          .arrow-up { animation: arrowRise 1.5s ease-in-out infinite; }
          .check-badge { animation: scaleIn 2s ease-in-out infinite; transform-origin: 140px 50px; }
        </style>
      </svg>
    `;
  }

  private getMaintenanceSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <style>
          ...
          @keyframes wrenchMove { 0%, 100% { transform: rotate(-15deg); } 50% { transform: rotate(-5deg); } }
          .gear { animation: gearRotate 4s linear infinite; transform-origin: 115px 75px; }
          .wrench { animation: wrenchMove 2s ease-in-out infinite; transform-origin: 94px 85px; }
        </style>
      </svg>
    `;
  }

  private getNoDataSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <style>
          ...
          .database-icon { animation: floatUpDown 3s ease-in-out infinite; }
        </style>
      </svg>
    `;
  }

  private getEmptySearchSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
  ...
  private getFavoritesSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <style>
          @keyframes heartBeat { 0%, 100% { transform: scale(1); } 10%, 30% { transform: scale(1.1); } 20%, 40% { transform: scale(0.95); } }
          @keyframes sparkleRotate { 0% { transform: rotate(0deg) scale(1); opacity: 0.6; } 50% { transform: rotate(180deg) scale(1.2); opacity: 1; } 100% { transform: rotate(360deg) scale(1); opacity: 0.6; } }
          .heart-main { animation: heartBeat 2s ease-in-out infinite; transform-origin: 100px 100px; }
          .sparkle-1 { animation: sparkleRotate 3s ease-in-out infinite; transform-origin: 60px 77px; }
          .sparkle-2 { animation: sparkleRotate 3s ease-in-out infinite 0.5s; transform-origin: 140px 92px; }
          .sparkle-3 { animation: sparkleRotate 3s ease-in-out infinite 1s; transform-origin: 75px 147px; }
        </style>
      </svg>
    `;
  }

  private getCalendarEventsSvg(): string {
    return `
      <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" class="illustration-svg">
        <defs>
        ...
        <style>
          @keyframes calendarFloat { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-6px); } }
          @keyframes plusFloat { 0%, 100% { transform: translate(0, 0) scale(1); } 50% { transform: translate(2px, -2px) scale(1.1); } }
          .calendar-float { animation: calendarFloat 3s ease-in-out infinite; }
          .plus-floating { animation: plusFloat 2s ease-in-out infinite; }
        </style>
      </svg>
    `;
  }
