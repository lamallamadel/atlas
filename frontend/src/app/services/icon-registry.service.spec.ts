...
describe('IconRegistryService', () => {
  ...
  it('should load icons from SVG sprite', async () => {
  ...
  it('should cache icons after loading', async () => {
    service.loadIcons().subscribe(() => {
  ...
  it('should get icon asynchronously', async () => {
  ...
  it('should share the same HTTP request for multiple subscribers', async () => {
    let count = 0;
  ...
  it('should search icons by name', () => {
    ...
    expect(
      results.some((icon) => icon.name.toLowerCase().includes('maison'))
    ).toBe(true);
  });
  ...
  it('should search icons by description', () => {
    ...
    expect(
      results.some((icon) =>
        icon.description.toLowerCase().includes('individuelle')
      )
    ).toBe(true);
  });
  ...
  it('should filter icons by category', () => {
    ...
    expect(houseTypes.every((icon) => icon.category === 'house-types')).toBe(
      true
    );
  });
  ...
  it('should return true for isLoaded after icons are loaded', async () => {
    service.loadIcons().subscribe(() => {
      expect(service.isLoaded()).toBe(true);
    });
